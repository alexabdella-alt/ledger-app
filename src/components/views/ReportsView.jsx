  ...
  React.useEffect(() => { setDrill(null); setDrillSel(null); }, [reportType]);
            ...
            return (
              <div>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap", marginBottom:24, padding:"12px 14px", background:"#F9FAFB", border:"1px solid #EAECF0", borderRadius:10 }}>
                  <span style={{ fontSize:13, fontWeight:600, color:"#344054" }}>Depreciation</span>
                  <span style={{ fontSize:13, color:"#667085" }}>Post all entries due through</span>
                  <input type="date" value={depRunDate} onChange={e=>setDepRunDate(e.target.value)} style={{ background:"#FFFFFF", border:"1px solid #D0D5DD", borderRadius:8, padding:"7px 10px", color:"#101828", fontSize:13, outline:"none" }} />
                  <button onClick={runDep} disabled={depRunning} style={{ padding:"8px 16px", borderRadius:8, fontSize:13, fontWeight:600, color:"#fff", background:depRunning?"#98A2B3":"#4F46E5", border:"none", cursor:depRunning?"default":"pointer" }}>{depRunning?"Posting…":"Run depreciation"}</button>
                  <span style={{ fontSize:12, color:"#98A2B3" }}>Straight-line · idempotent (only unposted months)</span>
                  {(isOwner || isAdmin) && <button onClick={()=>setAttachOpen(o=>!o)} style={{ marginLeft:"auto", fontSize:12, fontWeight:500, color:"#475467", background:"none", border:"none", cursor:"pointer" }}>{attachOpen?"Close":"Attach to existing asset"}</button>}
                </div>
                ...
                {(isOwner || isAdmin) && attachOpen && (
                  <div style={{ display:"flex", alignItems:"flex-end", gap:12, flexWrap:"wrap", marginBottom:24, padding:"12px 14px", background:"#FFFBF5", border:"1px solid #FEDF89", borderRadius:10 }}>
                    <label style={{ fontSize:12, color:"#344054", display:"flex", flexDirection:"column", gap:4 }}>Capitalization entry id
                      <input value={attachJe} onChange={e=>setAttachJe(e.target.value)} placeholder="journal_entries.id" style={{ width:320, height:36, borderRadius:8, border:"1px solid #D0D5DD", padding:"0 10px", fontSize:13, fontFamily:"monospace" }} /></label>
                    <label style={{ fontSize:12, color:"#344054", display:"flex", flexDirection:"column", gap:4 }}>Life (yrs)
                      <input type="number" min="1" value={attachLifeY} onChange={e=>setAttachLifeY(e.target.value)} style={{ width:80, height:36, borderRadius:8, border:"1px solid #D0D5DD", padding:"0 10px", fontSize:13 }} /></label>
                    <label style={{ fontSize:12, color:"#344054", display:"flex", flexDirection:"column", gap:4 }}>Salvage ($)
                      <input type="number" min="0" value={attachSalvage} onChange={e=>setAttachSalvage(e.target.value)} style={{ width:110, height:36, borderRadius:8, border:"1px solid #D0D5DD", padding:"0 10px", fontSize:13 }} /></label>
                    <label style={{ fontSize:12, color:"#344054", display:"flex", flexDirection:"column", gap:4 }}>In-service date
                      <input type="date" value={attachDate} onChange={e=>setAttachDate(e.target.value)} style={{ height:36, borderRadius:8, border:"1px solid #D0D5DD", padding:"0 10px", fontSize:13 }} /></label>
                    <button onClick={runAttach} disabled={attaching || !attachJe.trim()} style={{ height:36, padding:"0 16px", borderRadius:8, fontSize:13, fontWeight:600, color:"#fff", background:(attaching||!attachJe.trim())?"#98A2B3":"#DC6803", border:"none", cursor:(attaching||!attachJe.trim())?"default":"pointer" }}>{attaching?"Attaching…":"Attach depreciation"}</button>
                    <span style={{ fontSize:12, color:"#B54708", width:"100%" }}>Reuses the existing capitalization entry — posts no new Dr/Cr. In-service date defaults to the entry date if blank.</span>
                  </div>
                )}
                ...
                {reportType!=="monthly" && invoices.length===0 && <div style={{ color:"#475467", fontSize:14 }}>No data yet. Upload invoices or a bank statement to generate reports.</div>}
                        ...
                        <div style={{background:"#FFFFFF",border:"1px solid #E4E7EC",borderRadius:14,overflow:"hidden"}}>
                          ...
                          <div style={{padding:"24px 28px"}}>
                            ...
                            <div style={{display:"flex",justifyContent:"space-between",padding:"6px 0 6px 28px",borderBottom:"1px solid #F3F4F6"}}>
                              <div style={{fontSize:13,color:"#374151"}}>Retained Earnings, beginning of year</div>
                              <div style={{fontSize:13,fontFamily:"'DM Mono',monospace",color:beginningRE<0?"#D92D20":"#101828"}}>{beginningRE<0?"-":""}{bsFmt(Math.abs(beginningRE))}</div>
                            </div>
                            <div style={{display:"flex",justifyContent:"space-between",padding:"6px 0 6px 28px",borderBottom:"1px solid #F3F4F6"}}>
                              <div style={{fontSize:13,color:"#374151"}}>Net {bsCurrentNet>=0?"Income":"Loss"} (current period)</div>
                              <div style={{fontSize:13,fontFamily:"'DM Mono',monospace",color:bsCurrentNet>=0?"#039855":"#D92D20"}}>{bsCurrentNet<0?"-":""}{bsFmt(Math.abs(bsCurrentNet))}</div>
                            </div>
                            <div style={{display:"flex",justifyContent:"space-between",padding:"7px 0 7px 28px",borderBottom:"1px solid #D0D5DD",marginBottom:2}}>
                              ...
                              <div style={{fontSize:13,fontWeight:600,fontFamily:"'DM Mono',monospace",color:"#4F46E5"}}>{bsFmt(beginningRE+bsCurrentNet)}</div>
                            </div>
