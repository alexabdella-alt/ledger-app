...
export default function ReportsView() {
  ...
  React.useEffect(() => {
    ...
    setReportDateFrom(from); setReportDateTo(to); setReportRange("custom");
            ...
            // Cash flow by month — ACTUAL cash movements (CR-3): every entry whose LEG hits a
            // cash account, dated at the cash date, at the cash-leg amount, signed by direction.
            // Same basis as bank reconciliation (reconcile.js primitives), so the cash-flow
            // statement and the reconciliation can never disagree. Was: P&L rows + payment_status
            // flag (recognition date, ex-tax amount, ignoring the real settlement legs).
            const cfCashCodes = [...(cashGlCodes || [])].map(String);
            const cfIsCashLeg = c => c != null && cfCashCodes.includes(String(c));
            const byMonth = {};
            for (const inv of reconBooksSet(filtered, { cashCodes: cfCashCodes })) {
              if (!inv.date) continue;
              // Inter-account transfer (both legs cash) nets to zero for TOTAL cash — skip it.
              if (cfIsCashLeg(inv.gl_code) && cfIsCashLeg(inv.secondary_gl_code) && !String(inv.id).includes("_")) continue;
              const m = inv.date.slice(0,7);
              if (!byMonth[m]) byMonth[m] = { month:m, inflow:0, outflow:0 };
              const s = cashLegSigned(inv, cfCashCodes);
              if (s >= 0) byMonth[m].inflow += s; else byMonth[m].outflow += -s;
            }
            Object.values(byMonth).forEach(r => { r.inflow = Math.round(r.inflow*100)/100; r.outflow = Math.round(r.outflow*100)/100; });
            ...
            return (
              <div>
                <div style={{ marginBottom:24 }}>
                  ...
                  {/* C343 — O104's core promise: a report never sounds surer than the books. One
                ...
                {reportType!=="monthly" && invoices.length>0 && (
                  <div>
                    ...
                    {reportType==="vendor" && !drill && (() => {
                      // C344 — O104: "which suppliers are creeping up", from SIGNED months only.
                      const creep = vendorCreep({ rows: invoices, signedPeriods: activeSignedPeriods(signoffs) });
                      const copy = vendorCreepCopy(creep, { monthLabel, money: fmtMoney });
                      return (
                        <div data-vendor-creep style={{ marginBottom: 16, padding: "14px 18px", borderRadius: 12, background: "var(--sc-surface)", border: "1px solid var(--sc-border)" }}>
                          <div style={{ fontSize: 11, letterSpacing: 1.5, fontWeight: 600, color: "var(--sc-text-2)", marginBottom: 6 }}>CHARGING MORE THAN USUAL</div>
                          <div style={{ fontSize: 13.5, color: "var(--sc-text)" }}>{copy.headline}</div>
                          {copy.lines.map((l, i) => <div key={i} style={{ fontSize: 13, color: "var(--sc-text-2)", marginTop: 6 }}>• {l}</div>)}
                        </div>
                      );
                    })()}
                    {reportType==="vendor" && !drill && (
