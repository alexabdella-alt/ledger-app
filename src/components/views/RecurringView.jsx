            ...
            return (
              <div>
                <div style={{marginBottom:24}}>
                  <div style={{fontSize:10,letterSpacing:3,color:"#86868F",marginBottom:8}}>AUTOMATION</div>
                  <h1 style={{fontSize:28,fontWeight:600,margin:0,letterSpacing:-0.5}}>Recurring Transactions</h1>
                  <div style={{fontSize:13,color:"#86868F",marginTop:6}}>Set up transactions that repeat automatically. You can also tell the AI chat — "set up rent as $4,500/month starting June 1".</div>
                </div>
                {/* Due now alert */}
                {due.length>0 && (
                ...
                <div style={{background:"#141416",border:"1px solid #1C1C20",borderRadius:14,padding:20,marginBottom:20}}>
                  <div style={{fontSize:12,fontWeight:600,color:"#C7BFFF",marginBottom:14,letterSpacing:0.5}}>+ NEW RECURRING TRANSACTION</div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:12}}>
                    {[{k:"name",l:"Name",p:"e.g. Office Rent"},{k:"vendor",l:"Vendor",p:"Landlord name"},{k:"amount",l:"Amount ($)",p:"4500"}].map(f=>(
                      <div key={f.k}>
                        <div style={{fontSize:11,color:"#86868F",marginBottom:4}}>{f.l}</div>
                        <input value={newRec[f.k]} onChange={e=>setNewRec(d=>({...d,[f.k]:e.target.value}))} placeholder={f.p}
                          style={{width:"100%",boxSizing:"border-box",background:"#0C0C0E",border:"1px solid #262629",borderRadius:8,padding:"8px 10px",color:"#F2F2F4",fontSize:12,outline:"none"}}/>
                      </div>
                    ))}
                    <div>
                      <div style={{fontSize:11,color:"#86868F",marginBottom:4}}>GL Account</div>
                      <select value={newRec.gl_code} onChange={e=>{const a=CHART_OF_ACCOUNTS.find(x=>x.code===e.target.value);setNewRec(d=>({...d,gl_code:e.target.value,gl_name:a?.name||""}));}}
                        style={{width:"100%",background:"#0C0C0E",border:"1px solid #262629",borderRadius:8,padding:"8px 10px",color:"#F2F2F4",fontSize:12,outline:"none"}}>
                        {CHART_OF_ACCOUNTS.filter(a=>a.category==="Expenses").map(a=><option key={a.code} value={a.code}>{a.code} – {a.name}</option>)}
                      </select>
                    </div>
                    <div>
                      <div style={{fontSize:11,color:"#86868F",marginBottom:4}}>Frequency</div>
                    ...
                    <div>
                      <div style={{fontSize:11,color:"#86868F",marginBottom:4}}>First / Next Date</div>
                      <input type="date" value={newRec.next_date} onChange={e=>setNewRec(d=>({...d,next_date:e.target.value}))}
                        style={{width:"100%",boxSizing:"border-box",background:"#0C0C0E",border:"1px solid #262629",borderRadius:8,padding:"8px 10px",color:"#F2F2F4",fontSize:12,outline:"none"}}/>
                    </div>
                  </div>
                  <button onClick={addRecurring} disabled={!newRec.name||!newRec.amount} style={{padding:"9px 22px",borderRadius:9,fontSize:13,fontWeight:600,background:(!newRec.name||!newRec.amount)?"#1C1C20":"linear-gradient(135deg,#6D5EF6,#4A3DB8)",border:"none",color:"#F2F2F4",cursor:(!newRec.name||!newRec.amount)?"not-allowed":"pointer"}}>Save Recurring Transaction</button>
                </div>
                {/* List */}
                {recurring.length===0 ? (
                  <div style={{background:"#141416",border:"1px solid #1C1C20",borderRadius:14,padding:40,textAlign:"center",color:"#86868F",fontSize:13}}>No recurring transactions yet.</div>
                ) : (
                  <div style={{background:"#141416",border:"1px solid #1C1C20",borderRadius:14,overflow:"hidden"}}>
                    <table style={{width:"100%",borderCollapse:"collapse"}}>
                      <thead><tr style={{background:"#0C0C0E"}}>
                        {["Name","Vendor","Amount","GL","Frequency","Next Date",""].map(h=><th key={h} style={{padding:"11px 16px",textAlign:"left",fontSize:10,color:"#86868F",letterSpacing:1.2,fontWeight:500}}>{h}</th>)}
                      </tr></thead>
                      <tbody>
                        {recurring.map((r,i)=>{
                          const isDue = r.active && r.next_date && r.next_date<=today;
                          return (
                            <tr key={r.id} style={{borderTop:"1px solid #1C1C20",background:isDue?"#1A1000":i%2===0?"transparent":"#0A0A0C"}}>
                              <td style={{padding:"12px 16px"}}>
                                <div style={{fontSize:13,fontWeight:500}}>{r.name}</div>
                                {!r.active && <span style={{fontSize:10,color:"#86868F"}}>Paused</span>}
                                {isDue && <span style={{fontSize:10,color:"#F59E0B",marginLeft:6}}>Due today</span>}
                              </td>
                              <td style={{padding:"12px 16px",fontSize:13,color:"#9A9AA2"}}>{r.vendor||"—"}</td>
                              ...
                              <td style={{padding:"12px 16px"}}><span style={{fontSize:11,background:"#1C1C20",color:"#C7BFFF",borderRadius:20,padding:"2px 9px"}}>{r.gl_code} {r.gl_name}</span></td>
                              <td style={{padding:"12px 16px",fontSize:12,color:"#9A9AA2",textTransform:"capitalize"}}>{r.frequency}</td>
                              <td style={{padding:"12px 16px",fontSize:12,color:isDue?"#F59E0B":"#9A9AA2",fontFamily:"'DM Mono',monospace"}}>{r.next_date||"—"}</td>
                              <td style={{padding:"12px 16px"}}>
                                <div style={{display:"flex",gap:6}}>
                                  ...
                                  <button onClick={()=>setRecurring(prev=>prev.map(x=>x.id===r.id?{...x,active:!x.active}:x))} style={{padding:"5px 10px",borderRadius:7,fontSize:11,background:"transparent",border:"1px solid #262629",color:"#86868F",cursor:"pointer"}}>{r.active?"Pause":"Resume"}</button>
                                  <button onClick={()=>{setRecurring(prev=>prev.filter(x=>x.id!==r.id));logAudit("recurring_deleted",`Deleted: ${r.name}`);}} style={{padding:"5px 10px",borderRadius:7,fontSize:11,background:"transparent",border:"1px solid #262629",color:"#EF4444",cursor:"pointer"}}>×</button>
                                </div>
                              </td>
                            </tr>
                          );
