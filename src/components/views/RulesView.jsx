  ...
  return (
            <div>
              <div style={{ marginBottom:28 }}>
                ...
                <div style={{ fontSize:13, color:"#86868F", marginTop:6 }}>Rules auto-apply when invoices are uploaded. Create them by chatting with the AI assistant.</div>
              </div>
              {rules.length===0 ? (
                <div style={{ background:"#141416", border:"1px solid #1C1C20", borderRadius:14, padding:40, textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:13, color:"#86868F", marginBottom:20 }}>Tell the AI assistant things like "Always tag FedEx invoices to Shipping & Freight"</div>
                    ...
                    <thead>
                      <tr style={{ background:"#0C0C0E" }}>
                        {["Vendor","GL Account","Project",""].map(h=>(
                          <th key={h} style={{ padding:"13px 20px", textAlign:"left", fontSize:11, color:"#86868F", letterSpacing:1.5, fontWeight:500 }}>{h.toUpperCase()}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {rules.map((rule,i)=>(
                        <tr key={rule.vendor} style={{ borderTop:"1px solid #1C1C20", background:i%2===0?"transparent":"#0A0A0C" }}>
                          ...
                          <td style={{ padding:"14px 20px" }}><span style={{ background:"#1C1C20", padding:"4px 12px", borderRadius:20, fontSize:12, color:"#C7BFFF" }}>{rule.gl_code} · {rule.gl_name}</span></td>
                          <td style={{ padding:"14px 20px", fontSize:13, color:"#9A9AA2" }}>{rule.project||"—"}</td>
                          <td style={{ padding:"14px 20px" }}>
                            <button onClick={()=>setRules(r=>r.filter(x=>x.vendor!==rule.vendor))} style={{ background:"none", border:"1px solid #262629", color:"#EF4444", borderRadius:6, padding:"4px 10px", fontSize:12, cursor:"pointer" }}>Remove</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
