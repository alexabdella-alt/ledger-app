                        ...
                        <tr key={inv.id} style={{ borderTop:"1px solid var(--sc-border)", background:inv.status==="voided"?"var(--sc-error-soft)":i%2===0?"transparent":"var(--sc-bg)", opacity:inv.status==="voided"?0.5:1 }}>
                          ...
                          <td style={{ padding:"8px 16px" }}>
                            <div style={{ display:"flex", gap:4 }}>
                              {/* ── O130 — ONE CONTROL, and the system picks the mechanism. ──
                                  Was Void AND a red × side by side: two destructive buttons
                                  whose difference is bookkeeper vocabulary. `removalPlanFor`
                                  decides from the only input that matters — is the month
                                  signed off — and returns the sentence that matches its own
                                  decision, so the confirmation can never promise one outcome
                                  and perform the other (§9).
                                  An entry already dealt with gets a DISABLED control rather
                                  than one that refuses on click: refusing on click teaches
                                  that clicking is how you find out (O124/O126). */}
                              {inv.status !== "voided" && (() => {
                                ...
                                if (rev) return (
                                  <button disabled
                                    ...
                                    title={`Already removed${rev.date ? ` on ${fmtDate(rev.date)}` : ""}`}>
                                    Removed
                                  </button>
                                );
                                return (
                                  <button
                                    onClick={e=>{ e.stopPropagation();
                                      ...
                                      setDeleteConfirm({
                                        label: plan?.confirm || `Remove the entry for ${inv.vendor}?`,
                                        onConfirm: () => { removeEntry(inv); },
                                      }); }}
