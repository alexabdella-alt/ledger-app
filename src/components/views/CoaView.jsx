    ...
    <div>
      <div style={{ marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: "#475467", marginTop: 6 }}>Rename or renumber any account freely — the app tracks accounts by role, so reports and automations keep working. System accounts can be renamed but not deleted.</div>
        </div>
      ...
      {showAdd && (
        <div style={{ background: "#FFFFFF", border: "1px solid #4F46E533", borderRadius: 12, padding: 20, marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr 1fr auto", gap: 12, alignItems: "flex-end" }}>
            <div>
              <div style={{ fontSize: 11, color: "#475467", marginBottom: 4 }}>CODE</div>
              <input value={addDraft.code} onChange={e => setAddDraft(d => ({ ...d, code: e.target.value }))} placeholder="e.g. 6550"
                style={{ width: "100%", boxSizing: "border-box", background: "#F3F4F6", border: "1px solid #D0D5DD", borderRadius: 8, padding: "8px 10px", color: "#101828", fontSize: 13, outline: "none", fontFamily: "'DM Mono',monospace" }} />
            </div>
            <div>
              <div style={{ fontSize: 11, color: "#475467", marginBottom: 4 }}>NAME</div>
              <input value={addDraft.name} onChange={e => setAddDraft(d => ({ ...d, name: e.target.value }))} placeholder="e.g. Research & Development"
                style={{ width: "100%", boxSizing: "border-box", background: "#F3F4F6", border: "1px solid #D0D5DD", borderRadius: 8, padding: "8px 10px", color: "#101828", fontSize: 13, outline: "none" }} />
            </div>
            <div>
              <div style={{ fontSize: 11, color: "#475467", marginBottom: 4 }}>CATEGORY</div>
              <select value={addDraft.category} onChange={e => setAddDraft(d => ({ ...d, category: e.target.value }))}
                style={{ width: "100%", background: "#F3F4F6", border: "1px solid #D0D5DD", borderRadius: 8, padding: "8px 10px", color: "#101828", fontSize: 13, outline: "none" }}>
      ...
      {grouped.map(({ cat, accounts }) => (
        <div key={cat} style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 11, color: "#475467", letterSpacing: 2, marginBottom: 10, paddingLeft: 4 }}>{cat.toUpperCase()} — {accounts.length} accounts</div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <tbody>
                {accounts.map((acct, i) => {
                  ...
                  return (
                    <tr key={acct.db_id || acct.code} style={{ borderTop: i > 0 ? "1px solid #E4E7EC" : "none", background: isInactive ? "#F7F8FA" : i % 2 === 0 ? "transparent" : "#F7F8FA", opacity: isInactive ? 0.5 : 1 }}>
                      <td style={{ padding: "11px 16px", width: 80 }}>
                        {isEditing
                          ? <input value={editDraft.code} onChange={e => setEditDraft(d => ({ ...d, code: e.target.value }))}
                              style={{ width: 64, background: "#F3F4F6", border: "1px solid #4F46E5", borderRadius: 6, padding: "4px 8px", color: "#101828", fontSize: 12, outline: "none", fontFamily: "'DM Mono',monospace" }} />
                          : <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 12, color: "#475467" }}>{acct.code}</span>}
                      </td>
                      <td style={{ padding: "11px 16px" }}>
                        {isEditing
                          ? <input value={editDraft.name} onChange={e => setEditDraft(d => ({ ...d, name: e.target.value }))}
                              style={{ width: "100%", background: "#F3F4F6", border: "1px solid #4F46E5", borderRadius: 6, padding: "4px 8px", color: "#101828", fontSize: 13, outline: "none" }} />
                          : <span style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 13, fontWeight: 500, color: isInactive ? "#475467" : "#101828" }}>
                              {acct.name}
                              {isSystem && <span title="System account — rename freely but cannot be deleted" style={{ color: "#98A2B3", display: "inline-flex" }}>{lockIcon}</span>}
                            </span>}
                      </td>
                      <td style={{ padding: "11px 16px", width: 130 }}>
                        {isEditing
                          ? <select value={editDraft.category} onChange={e => setEditDraft(d => ({ ...d, category: e.target.value }))}
                              style={{ background: "#F3F4F6", border: "1px solid #4F46E5", borderRadius: 6, padding: "4px 6px", color: "#101828", fontSize: 12, outline: "none" }}>
                              {categories.map(c => <option key={c} value={c}>{c}</option>)}
                            </select>
                          : <span style={{ fontSize: 11, background: "#E4E7EC", color: "#475467", borderRadius: 20, padding: "2px 9px" }}>{acct.category}</span>}
                      </td>
                      <td style={{ padding: "11px 16px", width: 190 }}>
                        <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                          {isEditing ? (
                            <>
                              <button onClick={() => saveEdit(acct)} disabled={rowBusy} style={{ padding: "4px 12px", borderRadius: 7, fontSize: 11, fontWeight: 600, background: "linear-gradient(135deg,#D1FAE5,#039855)", border: "none", color: "#039855", cursor: rowBusy ? "default" : "pointer", opacity: rowBusy ? 0.6 : 1 }}>{rowBusy ? "Saving…" : "Save"}</button>
                              <button onClick={() => setEditingId(null)} disabled={rowBusy} style={{ padding: "4px 10px", borderRadius: 7, fontSize: 11, background: "transparent", border: "1px solid #D0D5DD", color: "#475467", cursor: "pointer" }}>×</button>
                            </>
                          ) : (
                            <>
                              <button onClick={() => startEdit(acct)} disabled={rowBusy} style={{ padding: "4px 12px", borderRadius: 7, fontSize: 11, background: "transparent", border: "1px solid #D0D5DD", color: "#475467", cursor: "pointer", opacity: rowBusy ? 0.6 : 1 }}>Edit</button>
                              <button onClick={() => toggleActive(acct)} disabled={rowBusy} style={{ padding: "4px 10px", borderRadius: 7, fontSize: 11, background: "transparent", border: "1px solid #D0D5DD", color: isInactive ? "#039855" : "#475467", cursor: "pointer", opacity: rowBusy ? 0.6 : 1 }}>{isInactive ? "Enable" : "Disable"}</button>
                              {isSystem
                                ? <span title="System account — cannot be deleted" style={{ padding: "4px 8px", color: "#D0D5DD", display: "inline-flex", alignItems: "center" }}>{lockIcon}</span>
                                : <button onClick={() => removeAccount(acct)} disabled={rowBusy} style={{ padding: "4px 10px", borderRadius: 7, fontSize: 11, background: "transparent", border: "1px solid #D92D2033", color: "#D92D20", cursor: rowBusy ? "default" : "pointer", opacity: rowBusy ? 0.6 : 1 }}>{rowBusy ? "…" : "Delete"}</button>}
                            </>
                          )}
                        </div>
                      </td>
