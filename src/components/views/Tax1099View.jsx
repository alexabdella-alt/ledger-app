...
export default function Tax1099View() {
  ...
  const statusOf = (v) => {
    ...
    if (exempt) return { key:"none", label:"Not required", color:"var(--sc-text-2)" };
    if (v.paid >= 600 && eligible) return c.sent_1099_2025 ? { key:"sent", label:"Sent ✓", color:"var(--sc-success)" } : { key:"needs", label:"Needs 1099", color:"var(--sc-error)" };
    if (v.paid >= 600 && (!c || !c.business_type)) return { key:"unknown", label:"Unknown", color:"var(--sc-warning)" };
    if (v.paid >= 1 && v.paid < 600) return { key:"tracking", label:"Tracking", color:"var(--sc-warning)" };
    return { key:"none", label:"Not required", color:"var(--sc-text-2)" };
  };
  ...
  return (
    <div>
      <div style={{ marginBottom:18 }}>
        <div style={{ fontSize:10, letterSpacing:3, color:"var(--sc-text-2)", marginBottom:8 }}>TAX · {taxYear}</div>
        <h1 style={{ fontSize:28, fontWeight:600, margin:0, letterSpacing:-0.5 }}>1099 contractors</h1>
      </div>

      {/* STEP 1 — education */}
      <div style={{ ...card, padding:"16px 20px", marginBottom:16, background:"var(--sc-gold-soft)", borderColor:"var(--sc-gold-soft)" }}>
        <div style={{ fontSize:13, color:"var(--sc-text-2)", lineHeight:1.6 }}>
      ...
      {(month===0 || month===11) && needs.length>0 && (
        <div style={{ ...card, padding:"14px 18px", marginBottom:16, background:"var(--sc-warning-soft)", borderColor:"var(--sc-warning-soft)", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10 }}>
          <div style={{ fontSize:13, fontWeight:600, color:"var(--sc-warning)" }}>⏰ 1099s are due January 31st — you have {needs.length} vendor{needs.length!==1?"s":""} who need one.</div>
          <button onClick={()=>exportCSV()} style={{ padding:"8px 16px", borderRadius:9, background:"var(--sc-warning)", border:"none", color:"var(--sc-on-accent)", fontSize:13, fontWeight:600, cursor:"pointer" }}>Export 1099 data →</button>
        </div>
      )}
      ...
      <div style={{ display:"flex", gap:12, alignItems:"center", marginBottom:16, flexWrap:"wrap" }}>
        <div style={{ ...card, padding:"12px 18px", flex:"1 1 200px" }}>
          <div style={{ fontSize:11, color:"var(--sc-text-2)", marginBottom:4 }}>NEED A 1099</div>
          <div style={{ fontSize:22, fontWeight:700, color:"var(--sc-error)" }}>{needs.length}</div>
        </div>
        <div style={{ ...card, padding:"12px 18px", flex:"1 1 200px" }}>
          <div style={{ fontSize:11, color:"var(--sc-text-2)", marginBottom:4 }}>YOUR EIN</div>
          <div style={{ fontSize:14, fontWeight:600, color: yourEIN?"var(--sc-text)":"var(--sc-error)" }}>{yourEIN || "Add in Settings → Company"}</div>
        </div>
        <button onClick={()=>exportCSV()} disabled={needs.length===0} style={{ padding:"11px 18px", borderRadius:10, background: needs.length?"var(--sc-gold)":"var(--sc-border)", border:"none", color: needs.length?"var(--sc-surface)":"var(--sc-text-mut)", fontSize:13, fontWeight:600, cursor: needs.length?"pointer":"not-allowed" }}>Export 1099 data (CSV)</button>
      </div>

      {/* STEP 2 — vendor list */}
      <div style={{ ...card, overflow:"hidden" }}>
        <div style={{ padding:"14px 18px", borderBottom:"1px solid var(--sc-surface-2)", fontSize:13, fontWeight:600 }}>Vendors paid in {taxYear}</div>
        {rows.length===0 ? <div style={{ padding:"40px", textAlign:"center", color:"var(--sc-text-2)", fontSize:13 }}>No vendor payments recorded for {taxYear} yet.</div> :
          rows.map(r=>{
            ...
            return (
              <div key={r.name} style={{ borderTop:"1px solid var(--sc-surface-2)" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, padding:"13px 18px", flexWrap:"wrap" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:11, minWidth:0 }}>
                    <span style={{ width:30, height:30, borderRadius:8, background:vendorColor(r.name), display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700, color:"var(--sc-on-accent)", flexShrink:0 }}>{initials(r.name)}</span>
                    <div style={{ minWidth:0 }}>
                      <div style={{ fontSize:14, fontWeight:500, color:"var(--sc-text)" }}>{r.name}</div>
                      <div style={{ fontSize:11, color:"var(--sc-text-2)" }}>{fmt(r.paid)} paid{c?.business_type?` · ${TYPE_OPTIONS.find(t=>t.v===c.business_type)?.label||c.business_type}`:""}</div>
                    </div>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
                    {pill(st.color, st.label)}
                    {st.key==="unknown" && <button onClick={()=>setEditing({ name:r.name, contact:c, business_type:"", ein_ssn:c?.ein_ssn||"", mailing_address:c?.mailing_address||"" })} style={{ padding:"7px 13px", borderRadius:8, fontSize:12, fontWeight:600, background:"var(--sc-warning-soft)", border:"1px solid var(--sc-warning-soft)", color:"var(--sc-warning)", cursor:"pointer" }}>Tell us about this vendor</button>}
                    {st.key==="needs" && <>
                      <button onClick={()=>setEditing({ name:r.name, contact:c, business_type:c?.business_type||"", ein_ssn:c?.ein_ssn||"", mailing_address:c?.mailing_address||"" })} style={{ padding:"7px 13px", borderRadius:8, fontSize:12, background:"var(--sc-surface)", border:"1px solid var(--sc-border-2)", color:"var(--sc-text-2)", cursor:"pointer" }}>Add missing info</button>
                      <button onClick={()=>exportCSV(r)} style={{ padding:"7px 13px", borderRadius:8, fontSize:12, background:"var(--sc-surface)", border:"1px solid var(--sc-border-2)", color:"var(--sc-text-2)", cursor:"pointer" }}>Export info</button>
                      <button onClick={()=>markSent(r)} style={{ padding:"7px 13px", borderRadius:8, fontSize:12, fontWeight:600, background:"var(--sc-success-soft)", border:"1px solid var(--sc-success-soft)", color:"var(--sc-success)", cursor:"pointer" }}>Mark as sent</button>
                    </>}
                  </div>
                </div>
                {/* STEP 4 — readiness checklist for needs-1099 */}
                {st.key==="needs" && (
                  <div style={{ padding:"0 18px 13px 59px", fontSize:12, color:"var(--sc-text-2)", display:"flex", gap:16, flexWrap:"wrap" }}>
                    <span>To send a 1099 you need:</span>
                    <span style={{ color: c?.ein_ssn?"var(--sc-success)":"var(--sc-error)" }}>{c?.ein_ssn?"☑":"☐"} Their SSN/EIN</span>
                    <span style={{ color: c?.mailing_address?"var(--sc-success)":"var(--sc-error)" }}>{c?.mailing_address?"☑":"☐"} Their address</span>
                    <span style={{ color: yourEIN?"var(--sc-success)":"var(--sc-error)" }}>{yourEIN?"☑":"☐"} Your EIN</span>
                  </div>
                )}
              </div>
            );
        ...
        <div onClick={()=>setEditing(null)} style={{ position:"fixed", inset:0, zIndex:10001, background:"rgba(17,24,39,0.35)", display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
          <div onClick={e=>e.stopPropagation()} className="sc-scale" style={{ ...card, padding:26, width:520, maxWidth:"94vw", maxHeight:"88vh", overflowY:"auto" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:18 }}>
              <div>
                <div style={{ fontSize:10, letterSpacing:1.5, color:"var(--sc-text-2)", marginBottom:6 }}>VENDOR DETAILS</div>
                <h2 style={{ fontSize:19, fontWeight:600, margin:0 }}>{editing.name}</h2>
              </div>
              <button onClick={()=>setEditing(null)} style={{ background:"none", border:"none", color:"var(--sc-text-2)", fontSize:24, cursor:"pointer", lineHeight:1 }}>×</button>
            </div>
            <div style={{ marginBottom:16 }}>
              <div style={lbl}>WHAT TYPE OF BUSINESS IS {editing.name.toUpperCase()}?</div>
              <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                {TYPE_OPTIONS.map(o=>(
                  <label key={o.v} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", borderRadius:9, border:`1px solid ${editing.business_type===o.v?"var(--sc-gold)":"var(--sc-border)"}`, background: editing.business_type===o.v?"var(--sc-gold-soft)":"var(--sc-surface)", cursor:"pointer", fontSize:13 }}>
                    <input type="radio" name="btype" checked={editing.business_type===o.v} onChange={()=>setEditing(s=>({...s,business_type:o.v}))} />
                    <span style={{ flex:1 }}>{o.label}</span>
                    <span style={{ fontSize:11, color: o.exempt?"var(--sc-text-2)":"var(--sc-error)", fontWeight:600 }}>{o.exempt?"No 1099 needed":"Needs 1099 if $600+"}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button disabled={!editing.business_type} onClick={saveVendor} style={{ flex:1, padding:"12px", borderRadius:10, border:"none", fontSize:14, fontWeight:600, cursor: editing.business_type?"pointer":"not-allowed", background: editing.business_type?"var(--sc-gold)":"var(--sc-border)", color: editing.business_type?"var(--sc-surface)":"var(--sc-text-mut)" }}>Save</button>
              <button onClick={()=>setEditing(null)} style={{ padding:"12px 18px", borderRadius:10, background:"var(--sc-surface)", border:"1px solid var(--sc-border-2)", color:"var(--sc-text-2)", fontSize:14, cursor:"pointer" }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
