            ...
            return (
              <div>
                <div style={{marginBottom:24}}>
                  ...
                  <div style={{fontSize:13,color:"#475467",marginTop:6}}>Export your data from QBO and upload here. AI maps their accounts to ours and imports everything.</div>
                </div>
                {/* Steps indicator */}
                <div style={{display:"flex",gap:0,marginBottom:28}}>
                  {[["upload","1. Upload"],["mapping","2. Review Mapping"],["done","3. Complete"]].map(([s,l],i,arr)=>(
                    <div key={s} style={{display:"flex",alignItems:"center"}}>
                      <div style={{padding:"6px 18px",borderRadius:20,fontSize:12,fontWeight:500,background:qboStep===s?"linear-gradient(135deg,#4F46E5,#4338CA)":["done","mapping"].includes(qboStep)&&i<["upload","mapping","done"].indexOf(qboStep)?"#03985522":"#E4E7EC",color:qboStep===s?"#fff":"#475467",border:"none"}}>{l}</div>
                      {i<arr.length-1 && <div style={{width:24,height:1,background:"#D0D5DD"}}/>}
                    </div>
                  ))}
                </div>
                {qboStep==="upload" && (
                  <div>
                    <div style={{background:"#FFFFFF",border:"1px solid #E4E7EC",borderRadius:14,padding:24,marginBottom:20}}>
                    ...
                    <div onDragOver={e=>{e.preventDefault();setQboDragOver(true);}} onDragLeave={()=>setQboDragOver(false)}
                      ...
                      style={{border:`2px dashed ${qboDragOver?"#4F46E5":"#D0D5DD"}`,borderRadius:14,padding:40,textAlign:"center",background:qboDragOver?"#EEF2FF":"#F3F4F6",transition:"all 0.2s",cursor:"pointer"}}
                      ...
                      {qboProcessing ? <div style={{color:"#4F46E5",fontSize:14}}>⏳ Reading your QBO data... mapping accounts...</div> : (
                        <div>
                          ...
                          <div style={{fontSize:12,color:"#475467"}}>CSV, Excel, IIF · AI reads the format automatically</div>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {qboStep==="mapping" && qboData && (
                  <div>
                    <div style={{background:"#FFFFFF",border:"1px solid #E4E7EC",borderRadius:14,padding:"14px 20px",marginBottom:16,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div style={{fontSize:13,color:"#475467"}}>Found <strong style={{color:"#101828"}}>{qboData.summary?.total_transactions||0} transactions</strong> from {qboData.summary?.date_range_start} to {qboData.summary?.date_range_end} · {qboData.summary?.total_vendors||0} vendors</div>
                      ...
                      <table style={{width:"100%",borderCollapse:"collapse"}}>
                        ...
                        <tbody>
                          {(qboData.source_accounts||[]).map((a,i)=>(
                            <tr key={a.qbo_name} style={{borderTop:"1px solid #E4E7EC",background:i%2===0?"transparent":"#F7F8FA"}}>
                              <td style={{padding:"11px 16px",fontSize:13,color:"#374151"}}>{a.qbo_name} <span style={{fontSize:11,color:"#475467"}}>({a.qbo_code})</span></td>
                              <td style={{padding:"11px 16px",color:"#475467"}}>→</td>
                              <td style={{padding:"11px 16px"}}>
                                <select value={qboMapping[a.qbo_name]||a.suggested_our_code||""} onChange={e=>setQboMapping(m=>({...m,[a.qbo_name]:e.target.value}))}
                                  style={{background:"#F3F4F6",border:"1px solid #D0D5DD",borderRadius:8,padding:"6px 10px",color:"#101828",fontSize:12,outline:"none",width:"100%"}}>
                ...
                {qboStep==="done" && (
                  <div style={{background:"#FFFFFF",border:"1px solid #03985533",borderRadius:14,padding:48,textAlign:"center"}}>
                    <div style={{fontSize:48,marginBottom:16}}>✓</div>
                    <div style={{fontSize:22,fontWeight:700,marginBottom:8,color:"#039855"}}>Import Complete</div>
                    <div style={{fontSize:14,color:"#475467",marginBottom:24}}>Your QBO data is now in your ledger, categorized and ready. Check the Audit Trail for a full import log.</div>
                    <div style={{display:"flex",gap:12,justifyContent:"center"}}>
                      ...
                      <button onClick={()=>setView("reports")} style={{padding:"10px 24px",borderRadius:10,fontSize:14,background:"#E4E7EC",border:"1px solid #D0D5DD",color:"#475467",cursor:"pointer"}}>View Reports →</button>
                    </div>
                  </div>
                )}
              </div>
