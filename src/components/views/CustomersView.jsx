...
export default function CustomersView() {
  ...
  // ★★ C317 — the same grouping key the vendor list and the summary use, aliases included.
  ...
  const cockpit = navSeat ? navSeat.isReviewerSeat : true;
            ...
            // ★★ C317 — same join, same fix, customer side. A customer billed under two
            // spellings had their revenue split between them, so one row showed part of what
            // they had actually been billed and the other showed the rest.
              ...
              return (
                <div>
                  ...
                  <div style={{ background:"var(--sc-surface)", border:"1px solid var(--sc-border)", borderRadius:14, padding:"16px 20px", marginBottom:20 }}>
                    <div style={{ fontSize:11, letterSpacing:1, color:"var(--sc-success)", fontWeight:600, marginBottom:12 }}>CONTACT INFO</div>
            ...
            // One row per customer KEY, labelled with the most recent spelling — the same
            // rule the vendor list uses, so two spellings can no longer become two customers.
            const custByKey = new Map();
            for (const i of invoices.filter(i=>glIsRevenue(i.gl_code)||i.type==="revenue")) {
              const k = i.vendor_key || keyOf(i.vendor);
              if (!k) continue;
              const prev = custByKey.get(k);
              if (!prev || String(i.date||"") >= String(prev.date||"")) custByKey.set(k, { name: i.vendor, date: i.date });
            }
            const ledgerCustomers = [...custByKey.values()].map(x => x.name);
            const ledgerOnlyCustomers = ledgerCustomers.filter(name =>
              !customerContacts.find(c => keyOf(c.name)===keyOf(name))
            );
            const allCustomers = [
            ...
            return (
              <div>
                <div style={{ marginBottom:24, display:"flex", justifyContent:"space-between", alignItems:"flex-end" }}>
                  <div>
                    <div style={{ fontSize:10, letterSpacing:3, color:"var(--sc-text-2)", marginBottom:8 }}>CUSTOMER MANAGEMENT</div>
                    <h1 style={{ fontSize:28, fontWeight:600, margin:0, letterSpacing:-0.5 }}>Customers</h1>
                    <div style={{ fontSize:13, color:"var(--sc-text-2)", marginTop:6 }}>Or just tell the AI chat — "Add Metro Cafe as a customer, they're on Net 15"</div>
                  </div>
                  <button onClick={()=>setChatOpen(true)} style={{ padding:"9px 18px", borderRadius:10, fontSize:13, background:"linear-gradient(135deg,var(--sc-success-soft),var(--sc-success))", border:"none", color:"var(--sc-success)", cursor:"pointer" }}>+ Add via Chat</button>
                </div>
                ...
                {allCustomers.length===0 ? (
                  <div style={{ background:"var(--sc-surface)", border:"1px solid var(--sc-border)", borderRadius:14, padding:48, textAlign:"center" }}>
                    ...
                    <div style={{ fontSize:13, color:"var(--sc-text-2)", marginBottom:20 }}>Customers appear when you upload revenue invoices, or tell the AI chat to add one.</div>
                    <button onClick={()=>setChatOpen(true)} style={{ background:"linear-gradient(135deg,var(--sc-success-soft),var(--sc-success))", border:"none", color:"var(--sc-success)", borderRadius:10, padding:"10px 24px", fontSize:13, cursor:"pointer" }}>Open AI Assistant</button>
                  </div>
                ) : (
                    ...
                    {allCustomers.map(c => {
                      ...
                      return (
                        <div key={c.id||c.name} style={{ background:"var(--sc-surface)", border:`1px solid ${overdueAR>0?"var(--sc-error-soft)":"var(--sc-border)"}`, borderRadius:14, overflow:"hidden" }}>
                          <div style={{ padding:"16px 20px", display:"flex", alignItems:"center", gap:14 }}>
                            ...
                            <div onClick={()=>setSelCustomer(c)} style={{ flex:1, minWidth:0, cursor:"pointer" }}>
                              <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                                ...
                                {c.fromContact && <span style={{ fontSize:10, background:"var(--sc-success-soft)", color:"var(--sc-success)", borderRadius:20, padding:"2px 7px" }}>Contact</span>}
                                {c.payment_terms && <span style={{ fontSize:10, background:"var(--sc-surface-2)", color:"var(--sc-text-2)", borderRadius:20, padding:"2px 7px", border:"1px solid var(--sc-border-2)" }}>{c.payment_terms}</span>}
                                {overdueAR>0 && <span style={{ fontSize:10, background:"var(--sc-error-soft)", color:"var(--sc-error)", borderRadius:20, padding:"2px 7px" }}>Overdue</span>}
                                {(c.tags||[]).map(t=><span key={t} style={{ fontSize:10, background:"var(--sc-border)", color:"var(--sc-text-2)", borderRadius:20, padding:"2px 7px" }}>{t}</span>)}
                              </div>
                            ...
                            <div style={{ display:"flex", gap:10, alignItems:"center", flexShrink:0 }}>
                              {billedYTD>0 && <div style={{ textAlign:"right" }}>
                                <div style={{ fontSize:11, color:"var(--sc-text-2)" }}>BILLED YTD</div>
                          ...
                          {!isEditing && c.notes && <div style={{ padding:"10px 20px", borderTop:"1px solid var(--sc-border)", fontSize:12, color:"var(--sc-text-2)" }}>📝 {c.notes}</div>}
                          {!isEditing && (c.min_expected||c.max_expected) && (
                            <div style={{ padding:"10px 20px", borderTop:"1px solid var(--sc-border)", fontSize:12, color:"var(--sc-text-2)" }}>
                              Expected revenue: <span style={{ color:"var(--sc-success)" }}>{fmt(c.min_expected||0)} – {fmt(c.max_expected||0)}/invoice</span>
                            </div>
                          )}
                        </div>
                      );
