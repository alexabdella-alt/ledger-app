            ...
            return (
              <div>
                <div style={{ marginBottom:24, display:"flex", justifyContent:"space-between", alignItems:"flex-end" }}>
                  <div>
                    <div style={{ fontSize:10, letterSpacing:3, color:"#86868F", marginBottom:8 }}>CUSTOMER MANAGEMENT</div>
                    <h1 style={{ fontSize:28, fontWeight:600, margin:0, letterSpacing:-0.5 }}>Customers</h1>
                    <div style={{ fontSize:13, color:"#86868F", marginTop:6 }}>Or just tell the AI chat — "Add Metro Cafe as a customer, they're on Net 15"</div>
                  </div>
                ...
                {allCustomers.length===0 ? (
                  <div style={{ background:"#141416", border:"1px solid #1C1C20", borderRadius:14, padding:48, textAlign:"center" }}>
                    ...
                    <div style={{ fontSize:13, color:"#86868F", marginBottom:20 }}>Customers appear when you upload revenue invoices, or tell the AI chat to add one.</div>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                    {allCustomers.map(c => {
                      ...
                      return (
                        <div key={c.id||c.name} style={{ background:"#141416", border:`1px solid ${overdueAR>0?"#EF444433":"#1C1C20"}`, borderRadius:14, overflow:"hidden" }}>
                          <div style={{ padding:"16px 20px", display:"flex", alignItems:"center", gap:14 }}>
                            ...
                            <div style={{ flex:1, minWidth:0 }}>
                              <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                                ...
                                {c.payment_terms && <span style={{ fontSize:10, background:"#0C0C0E", color:"#9A9AA2", borderRadius:20, padding:"2px 7px", border:"1px solid #262629" }}>{c.payment_terms}</span>}
                                {overdueAR>0 && <span style={{ fontSize:10, background:"#EF444422", color:"#EF4444", borderRadius:20, padding:"2px 7px" }}>Overdue</span>}
                                {(c.tags||[]).map(t=><span key={t} style={{ fontSize:10, background:"#1C1C20", color:"#86868F", borderRadius:20, padding:"2px 7px" }}>{t}</span>)}
                              </div>
                            ...
                            <div style={{ display:"flex", gap:10, alignItems:"center", flexShrink:0 }}>
                              {totalRevenue>0 && <div style={{ textAlign:"right" }}>
                                <div style={{ fontSize:11, color:"#86868F" }}>TOTAL REVENUE</div>
                              ...
                              {openAR>0 && <div style={{ textAlign:"right" }}>
                                <div style={{ fontSize:11, color:"#86868F" }}>OPEN AR</div>
                                <div style={{ fontSize:16, fontWeight:700, fontFamily:"'DM Mono',monospace", color:overdueAR>0?"#EF4444":"#F59E0B" }}>{fmt(openAR)}</div>
                              </div>}
                              <button onClick={()=>isEditing?saveEdit(c):startEdit(c)} style={{ padding:"7px 14px", borderRadius:8, fontSize:12, background:isEditing?"linear-gradient(135deg,#065F46,#047857)":"#1C1C20", border:"1px solid #262629", color:isEditing?"#6EE7B7":"#9A9AA2", cursor:"pointer" }}>
                                {isEditing?"Save":"Edit"}
                              </button>
                          ...
                          {!isEditing && c.notes && <div style={{ padding:"10px 20px", borderTop:"1px solid #1C1C20", fontSize:12, color:"#9A9AA2" }}>📝 {c.notes}</div>}
