...
import { computeAR, isLiveEntry } from "../../lib/reports";
...
export default function ArView() {
  const { AP_PRIORITY, CHART_OF_ACCOUNTS, CONTRACT_TYPES, activeRecon, aiStep, aiSuggestion, allProjects, allVendorNames, apAgingLoading, apAgingNarration, apSettings, apView, applyMatch, applyRule, approveInvoice, arAgingLoading, arAgingNarration, arView, auditActionFilter, auditLog, auditSearch, bankAccounts, bankDragOver, bankFileName, bankProcessing, bankProgress, bankStep, bankTransactions, basisMode, basisNarration, basisNarrationLoading, bookBankTransactions, bookToDb, cashBalance, chatBottomRef, chatHistory, chatInput, chatInputRef, chatLoading, chatOpen, checkRunMode, checkWatchTriggers, clarificationQueue, classifyFile, coaAddDraft, coaEditDraft, coaEditingCode, coaShowAdd, companies, companySettings, contacts, contractDragOver, contractProcessing, contractView, contracts, currentCompany, customCOA, customProjects, customersEditDraft, customersEditingId, deleteConfirm, deleteJournalEntry, dismissMatch, docLibrary, docsFilterType, docsPreview, dragOver, fileStoreRef, fileToBase64, filteredInvoices, form, getOpenAP, getOpenAR, getUnpaidInvoices, getUnpaidReceivables, glBreakdown, handleBankFile, handleBookInvoice, handleChatSend, handleContractFile, handleFileSelect, handleFormChange, handleUniversalUpload, hasUnread, inputStyle, invoices, isAILoading, labelStyle, loadAllData, loadContractsFromDB, logAudit, mainContentRef, markPaid, markBillPaid, matchHistory, matchProcessing, matchQueue, netIncome, notification, onNewCompany, onSignOut, onSwitchCompany, onViewChange, openingBalAsOfDate, openingBalBalances, openingBalances, payrollDragOver, payrollImports, payrollProcessing, persistContact, persistContract, persistJournalEntry, persistRecode, persistedView, postAllContractEntries, postContractEntry, processUploadItem, qboData, qboDragOver, qboMapping, qboPreview, qboProcessing, qboStep, reconAccount, reconSessions, reconStatementBalance, recurring, recurringNewRec, rejectInvoice, reportDateFrom, reportDateTo, reportRange, reportType, rules, runAPEngine, runAPScreen, runFullAI, runMatchingEngine, selectedContract, selectedInvoice, selectedPayments, sendInvoiceDraftState, sendInvoiceShowPreview, sentInvoiceDraft, sentInvoices, session, setActiveRecon, setAiStep, setAiSuggestion, setApAgingLoading, setApAgingNarration, setApView, setArAgingLoading, setArAgingNarration, setArView, setAuditActionFilter, setAuditLog, setAuditSearch, setBankAccounts, setBankDragOver, setBankFileName, setBankProcessing, setBankProgress, setBankStep, setBankTransactions, setBasisMode, setBasisNarration, setBasisNarrationLoading, setCashBalance, setChatHistory, setChatInput, setChatLoading, setChatOpen, setCheckRunMode, setClarificationQueue, setCoaAddDraft, setCoaEditDraft, setCoaEditingCode, setCoaShowAdd, setCompanySettings, setContacts, setContractDragOver, setContractProcessing, setContractView, setContracts, setCustomCOA, setCustomProjects, setCustomersEditDraft, setCustomersEditingId, setDeleteConfirm, setDocLibrary, setDocsFilterType, setDocsPreview, setDragOver, setForm, setHasUnread, setInvoices, setIsAILoading, setMatchHistory, setMatchProcessing, setMatchQueue, setNotification, setOpeningBalAsOfDate, setOpeningBalBalances, setOpeningBalances, setPayrollDragOver, setPayrollImports, setPayrollProcessing, setQboData, setQboDragOver, setQboMapping, setQboPreview, setQboProcessing, setQboStep, setReconAccount, setReconSessions, setReconStatementBalance, setRecurring, setRecurringNewRec, setReportDateFrom, setReportDateTo, setReportRange, setReportType, setRules, setSelectedContract, setSelectedInvoice, setSelectedPayments, setSendInvoiceDraftState, setSendInvoiceShowPreview, setSentInvoiceDraft, setSentInvoices, setSettingsDraft, setSettingsLogoPreview, setSettingsSaved, setUniversalDragOver, setUnknownDocs, setUploadProcessing, setUploadQueue, setUploadedFile, setVendorFilter, setVendorsEditDraft, setVendorsEditingId, setVendorsSelectedContact, setView, setViewRaw, settingsDraft, settingsLogoPreview, settingsSaved, showNotification, storeDocument, supabase, totalExpenses, totalRevenue, universalDragOver, unknownDocs, uploadActiveRef, uploadProcessing, uploadQueue, uploadedFile, vendorFilter, vendorSummary, vendorsEditDraft, vendorsEditingId, vendorsSelectedContact, view } = useERP();
            ...
            // All revenue invoices = AR (live entries only — voided/deleted excluded, matching canonical).
            const arAll   = invoices.filter(i => isLiveEntry(i) && (glIsRevenue(i.gl_code) || i.type==="revenue"));
            ...
            const totalAR = computeAR(invoices).total;   // canonical — matches dashboard, reports, AI
            ...
            return (
              <div>
                ...
                <div style={{ marginBottom:24 }}>
                  ...
                  <div style={{ fontSize:13, color:"#475467", marginTop:6 }}>Outstanding invoices you've issued to customers.</div>
                </div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
                  {[
                    ...
                    <div key={c.label} style={{ background:"#FFFFFF", border:"1px solid #E4E7EC", borderRadius:12, padding:"16px 18px" }}>
                      ...
                      <div style={{ fontSize:11, color:"#475467", marginTop:4 }}>{c.sub}</div>
                    </div>
                  ))}
                </div>

                {/* Tab bar */}
                <div style={{ display:"flex", gap:2, background:"#F3F4F6", borderRadius:10, padding:3, border:"1px solid #E4E7EC", marginBottom:20, width:"fit-content" }}>
                  {[["inbox","📥 Inbox"],["collections","📞 Collections"],["aging","📊 Aging"]].map(([id,label])=>(
                    <button key={id} onClick={()=>setArView(id)} style={{ padding:"8px 18px", borderRadius:8, fontSize:13, fontWeight:arView===id?600:400,
                      background:arView===id?"#E4E7EC":"transparent", border:"none", color:arView===id?"#039855":"#475467", cursor:"pointer",
                      display:"flex", alignItems:"center", gap:6 }}>
                      {label}
                      {id==="collections"&&collectionsQueue.length>0&&<span style={{ background:"#D92D20", color:"#fff", borderRadius:20, fontSize:10, fontWeight:700, padding:"1px 6px" }}>{collectionsQueue.length}</span>}
                    </button>
                  ))}
                </div>
                  ...
                  <div>
                    {arAll.length===0 ? (
                      <div style={{ background:"#FFFFFF", border:"1px solid #E4E7EC", borderRadius:14, padding:48, textAlign:"center" }}>
                        ...
                        <div style={{ fontSize:13, color:"#475467" }}>Upload invoices you've sent to customers — they'll appear here as outstanding receivables.</div>
                      </div>
                    ) : (
                      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                        {[...arAll].sort((a,b) => {
                          ...
                          return (
                            <div key={inv.id} style={{ background:"#FFFFFF", border:`1px solid ${isCollected?"#E4E7EC":isOverdue?"#D92D2033":"#E4E7EC"}`, borderRadius:14, overflow:"hidden", opacity:isCollected?0.5:1 }}>
                              <div style={{ padding:"16px 20px", display:"flex", alignItems:"center", gap:14 }}>
                                ...
                                <div style={{ flex:1, minWidth:0 }}>
                                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3, flexWrap:"wrap" }}>
                                    ...
                                    {isCollected && <span style={{ fontSize:11, background:"#03985522", color:"#039855", borderRadius:20, padding:"2px 8px" }}>✓ Collected</span>}
                              ...
                              {!isCollected && (
                                <div style={{ padding:"10px 20px", borderTop:"1px solid #E4E7EC", background:"#F3F4F6", display:"flex", gap:8 }}>
                                  <button onClick={()=>markCollected(inv.id)} style={{ padding:"6px 16px", borderRadius:8, fontSize:12, fontWeight:600, background:"linear-gradient(135deg,#D1FAE5,#039855)", border:"none", color:"#039855", cursor:"pointer" }}>✓ Mark Collected</button>
                                  <button style={{ padding:"6px 14px", borderRadius:8, fontSize:12, background:"transparent", border:"1px solid #D0D5DD", color:"#475467", cursor:"pointer" }} onClick={()=>setArView("collections")}>Follow Up →</button>
                                </div>
                              )}
                            </div>
                          );
                ...
                {arView==="collections" && (
                  <div>
                    <div style={{ fontSize:13, color:"#475467", marginBottom:16 }}>Overdue invoices sorted by amount — largest first.</div>
                    {collectionsQueue.length===0 ? (
                      <div style={{ background:"#FFFFFF", border:"1px solid #E4E7EC", borderRadius:12, padding:32, textAlign:"center", color:"#475467", fontSize:13 }}>
                        ...
                        <table style={{ width:"100%", borderCollapse:"collapse" }}>
                          <thead><tr style={{ background:"#F3F4F6" }}>
                            {["Customer","Invoice Date","Due Date","Days Overdue","Amount","Action"].map(h=>(
                              <th key={h} style={{ padding:"10px 16px", textAlign:"left", fontSize:11, color:"#475467", letterSpacing:1.2, fontWeight:500 }}>{h}</th>
                            ))}
                          </tr></thead>
                          <tbody>
                            {collectionsQueue.map((inv,i) => {
                              const daysOverdue = Math.floor((new Date(today)-new Date(inv.due_date))/86400000);
                              const urgencyColor = daysOverdue>90?"#FEE2E2":daysOverdue>60?"#D92D20":daysOverdue>30?"#DC6803":"#4F46E5";
                              return (
                                  ...
                                  <td style={{ padding:"13px 16px" }}>
                                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                                      ...
                                      <div>
                                        ...
                                        <div style={{ fontSize:11, color:"#475467" }}>{inv.description?.slice(0,35)}</div>
                                      </div>
                                    </div>
                                  </td>
                ...
                {arView==="aging" && (
                  <div>
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
                      {[
                        { label:"Current (0–30d)", bucket:aging.current, color:"#039855" },
                        { label:"31–60 Days",      bucket:aging.d60,     color:"#DC6803" },
                        { label:"61–90 Days",      bucket:aging.d90,     color:"#D92D20" },
                        ...
                        <div key={label} style={{ background:"#FFFFFF", border:`1px solid ${color}33`, borderRadius:12, padding:"16px 18px" }}>
                          <div style={{ fontSize:11, color:"#475467", marginBottom:8 }}>{label}</div>
                          <div style={{ fontSize:24, fontWeight:700, fontFamily:"'DM Mono',monospace", color }}>{fmt(bucket.total)}</div>
                          <div style={{ fontSize:11, color:"#475467", marginTop:4 }}>{bucket.count} invoice{bucket.count!==1?"s":""}</div>
                          <div style={{ marginTop:10, height:3, background:"#E4E7EC", borderRadius:2 }}>
                    ...
                    <div style={{ background:"#FFFFFF", border:"1px solid #E4E7EC", borderRadius:14, padding:20, marginBottom:20 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:arAgingNarration||arAgingLoading?16:0 }}>
                        ...
                        <button onClick={handleArAging} disabled={arAgingLoading}
                          style={{ padding:"7px 16px", borderRadius:8, fontSize:12, background:"linear-gradient(135deg,#D1FAE5,#039855)", border:"none", color:"#039855", cursor:arAgingLoading?"wait":"pointer" }}>
                          {arAgingLoading?"⟳ Analyzing...":"Generate Analysis"}
                        </button>
                      </div>
                      {arAgingLoading && <div style={{ display:"flex", gap:5 }}>{[0,1,2].map(i=><div key={i} style={{ width:6,height:6,borderRadius:"50%",background:"#475467",animation:`pulse 1.2s ease-in-out ${i*0.2}s infinite` }} />)}</div>}
                      {arAgingNarration && <div style={{ fontSize:13, color:"#374151", lineHeight:1.8, whiteSpace:"pre-wrap" }}>{arAgingNarration}</div>}
                      {!arAgingNarration && !arAgingLoading && <div style={{ fontSize:13, color:"#475467" }}>Click Generate Analysis for AI commentary on your AR position and collection risk.</div>}
                    </div>
                        ...
                        <table style={{ width:"100%", borderCollapse:"collapse" }}>
                          ...
                          <tbody>
                            {[...arOpen].sort((a,b)=>(a.date||"").localeCompare(b.date||"")).map((inv,i) => {
                              ...
                              const ageColor = ageDays<=30?"#039855":ageDays<=60?"#DC6803":ageDays<=90?"#D92D20":"#FEE2E2";
                              return (
                                <tr key={inv.id} style={{ borderTop:"1px solid #E4E7EC", background:i%2===0?"transparent":"#F7F8FA" }}>
