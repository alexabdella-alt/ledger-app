            ...
            return (
              <div>
                ...
                <div style={{ marginBottom:24 }}>
                  ...
                  <div style={{ fontSize:13, color:"#86868F", marginTop:6 }}>Outstanding invoices you've issued to customers.</div>
                </div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
                  {[
                    ...
                    { label:"Current (0–30d)",    value:fmt(aging.current.total), sub:`${aging.current.count} invoices`,     color:"#C7BFFF" },
                    { label:"Collected (Total)",  value:fmt(arAll.filter(i=>i.payment_status==="collected"||i.payment_status==="paid").reduce((s,i)=>s+i.amount,0)), sub:`${arAll.filter(i=>i.payment_status==="collected"||i.payment_status==="paid").length} invoices`, color:"#86868F" },
                  ].map(c=>(
                ...
                <div style={{ display:"flex", gap:2, background:"#0C0C0E", borderRadius:10, padding:3, border:"1px solid #1C1C20", marginBottom:20, width:"fit-content" }}>
                  {[["inbox","📥 Inbox"],["collections","📞 Collections"],["aging","📊 Aging"]].map(([id,label])=>(
                    <button key={id} onClick={()=>setArView(id)} style={{ padding:"8px 18px", borderRadius:8, fontSize:13, fontWeight:arView===id?600:400,
                      background:arView===id?"#1C1C20":"transparent", border:"none", color:arView===id?"#10B981":"#86868F", cursor:"pointer",
                  ...
                  <div>
                    {arAll.length===0 ? (
                      <div style={{ background:"#141416", border:"1px solid #1C1C20", borderRadius:14, padding:48, textAlign:"center" }}>
                        ...
                        <div style={{ fontSize:13, color:"#86868F" }}>Upload invoices you've sent to customers — they'll appear here as outstanding receivables.</div>
                      </div>
                    ) : (
                      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                        {[...arAll].sort((a,b) => {
                          ...
                          return (
                            <div key={inv.id} style={{ background:"#141416", border:`1px solid ${isCollected?"#1C1C20":isOverdue?"#EF444433":"#1C1C20"}`, borderRadius:14, overflow:"hidden", opacity:isCollected?0.5:1 }}>
                              <div style={{ padding:"16px 20px", display:"flex", alignItems:"center", gap:14 }}>
                                ...
                                <div style={{ flex:1, minWidth:0 }}>
                                  ...
                                  <div style={{ fontSize:12, color:"#9A9AA2" }}>{inv.description} · {inv.gl_name} · {inv.date}
                                    {inv.payment_terms && <span style={{ color:"#86868F", marginLeft:8 }}>{inv.payment_terms}</span>}
                                  </div>
                                </div>
                              ...
                              {!isCollected && (
                                <div style={{ padding:"10px 20px", borderTop:"1px solid #1C1C20", background:"#0C0C0E", display:"flex", gap:8 }}>
                                  ...
                                  <button style={{ padding:"6px 14px", borderRadius:8, fontSize:12, background:"transparent", border:"1px solid #262629", color:"#86868F", cursor:"pointer" }} onClick={()=>setArView("collections")}>Follow Up →</button>
                                </div>
                              )}
                            </div>
                          );
                ...
                {arView==="collections" && (
                  <div>
                    <div style={{ fontSize:13, color:"#86868F", marginBottom:16 }}>Overdue invoices sorted by amount — largest first.</div>
                        ...
                        <table style={{ width:"100%", borderCollapse:"collapse" }}>
                          <thead><tr style={{ background:"#0C0C0E" }}>
                            {["Customer","Invoice Date","Due Date","Days Overdue","Amount","Action"].map(h=>(
                              <th key={h} style={{ padding:"10px 16px", textAlign:"left", fontSize:11, color:"#86868F", letterSpacing:1.2, fontWeight:500 }}>{h}</th>
                            ))}
                          </tr></thead>
                          <tbody>
                            {collectionsQueue.map((inv,i) => {
                              const daysOverdue = Math.floor((new Date(today)-new Date(inv.due_date))/86400000);
                              const urgencyColor = daysOverdue>90?"#7F1D1D":daysOverdue>60?"#EF4444":daysOverdue>30?"#F59E0B":"#C7BFFF";
                              return (
                                  ...
                                  <td style={{ padding:"13px 16px" }}>
                                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                                      ...
                                      <div>
                                        ...
                                        <div style={{ fontSize:11, color:"#86868F" }}>{inv.description?.slice(0,35)}</div>
                                      </div>
                                    </div>
                                  </td>
                                  <td style={{ padding:"13px 16px", fontSize:12, color:"#9A9AA2" }}>{inv.date}</td>
                ...
                {arView==="aging" && (
                  <div>
                    ...
                    <div style={{ background:"#141416", border:"1px solid #1C1C20", borderRadius:14, padding:20, marginBottom:20 }}>
                      ...
                      {arAgingLoading && <div style={{ display:"flex", gap:5 }}>{[0,1,2].map(i=><div key={i} style={{ width:6,height:6,borderRadius:"50%",background:"#86868F",animation:`pulse 1.2s ease-in-out ${i*0.2}s infinite` }} />)}</div>}
                      {arAgingNarration && <div style={{ fontSize:13, color:"#D2D2D6", lineHeight:1.8, whiteSpace:"pre-wrap" }}>{arAgingNarration}</div>}
                      {!arAgingNarration && !arAgingLoading && <div style={{ fontSize:13, color:"#86868F" }}>Click Generate Analysis for AI commentary on your AR position and collection risk.</div>}
                    </div>

                    {/* Aging detail table */}
                    {arOpen.length>0 && (
                      <div style={{ background:"#141416", border:"1px solid #1C1C20", borderRadius:14, overflow:"hidden" }}>
                        <div style={{ padding:"14px 20px", borderBottom:"1px solid #1C1C20", fontSize:13, fontWeight:600 }}>All Open Receivables</div>
                        <table style={{ width:"100%", borderCollapse:"collapse" }}>
                          <thead><tr style={{ background:"#0C0C0E" }}>
