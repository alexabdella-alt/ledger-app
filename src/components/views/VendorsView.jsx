...
export default function VendorsView() {
  ...
  // ★★★ C316 — THE 1099 BADGE NOW SHOWS THE DERIVED ANSWER, AND IT IS NOT A BUTTON.
  // It used to toggle `contacts.is_1099` — a field that, since C256 replaced the flag with
  // a derivation, IS READ BY NOTHING. Not the Tax page, not `form1099.js`, not the AI
  // context: all three read `is_1099_exempt` and `business_type` instead. So the badge was
  // a control that wrote a value nobody consults, on the one subject in this product filed
  // with the IRS under the accountant's name.
  //
  // ★★ AND IT COULD CONTRADICT THE PAGE THAT DOES DECIDE. A food supplier with the badge
  // ticked reads "1099 ✓" here while the Tax page says "paid for goods, which aren't
  // reported on a 1099 at any amount." Same question, two screens, opposite answers — §9's
  // describe-from-the-record rule, with a filing on the end of it.
  ...
  const verdict1099 = React.useCallback((vendor) => {
    ...
    const k = keyOf(vendor.name);
    const rows = (invoices || []).filter(i => (i.vendor_key || keyOf(i.vendor)) === k);
    return verdictFor(vendor, reportablePayments(rows, roleOfCode));
  }, [invoices, roleOfCode, keyOf]);
  const badge1099 = (v) => {
    const r = verdict1099(v);
    if (!r) return null;
    if (r.verdict === VERDICT.ELIGIBLE) return { text: "Needs a 1099", tone: "var(--sc-warning)" };
    if (r.verdict === VERDICT.NEEDS_INFO) return { text: "1099 — needs info", tone: "var(--sc-gold)" };
    return null;   // below the floor, goods only, or exempt: nothing to say
  };
            ...
            // C453 — the same flag-derived openness C452 removed from Customers, on the supplier
            // side: "Bills you still owe" was every expense row without payment_status = paid, so a
            // card purchase at the till (Dr Expense / Cr Cash, no status) read as a bill still owed
            // — and "Paid YTD" counted ONLY rows stamped paid, so that same purchase counted as
            // paid nowhere. Open = the A/P-leg rule the Bills screen uses (openPayablesGL, C309);
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:20 }}>
                    ...
                    <div>
                      ...
                      <div style={{ display:"flex", gap:8, marginTop:6, alignItems:"center" }}>
                        ...
                        {(() => { const b = badge1099(v); const why = verdict1099(v)?.why; return b && <span title={why || undefined} style={{ fontSize:11, color:b.tone, border:`1px solid ${b.tone}44`, background:b.tone+"18", borderRadius:20, padding:"3px 10px" }}>{b.text}</span>; })()}
                      </div>
                    </div>
                  </div>
                          ...
                          <div style={{ padding:"16px 20px", display:"flex", alignItems:"center", gap:14 }}>
                            ...
                            <div onClick={()=>setSelectedContact(v)} style={{ flex:1, minWidth:0, cursor:"pointer" }}>
                              <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                                ...
                                {(() => { const b = badge1099(v); return b && <span style={{ fontSize:10, background:b.tone+"18", color:b.tone, borderRadius:20, padding:"2px 7px", border:`1px solid ${b.tone}44` }}>{b.text}</span>; })()}
