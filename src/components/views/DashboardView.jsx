              ...
              {clarificationQueue.length > 0 && (
                <div id="clarification-section" style={{ marginBottom:24, background:"#FEF3C7", border:"2px solid #D97706", borderRadius:16, padding:20 }}>
                  ...
                  {clarificationQueue.map(item => (
                    <div key={item.id} style={{ background: item.gaap ? "#EEF2FF" : item.isDuplicate ? "#FEF2F2" : "#FEF3C7", border: `1px solid ${item.gaap ? "#4F46E544" : item.isDuplicate ? "#DC262644" : "#D9770644"}`, borderRadius:14, padding:20, marginBottom:12 }}>
                      {item.gaap ? (
                        ...
                        <>
                          ...
                          <button onClick={()=>{ logAudit("invoice_rejected", `Rejected (GAAP review): ${item.invoice.vendor} · $${(item.invoice.amount||0).toFixed(2)}`, item.invoice, null); setClarificationQueue(prev=>prev.filter(c=>c.id!==item.id)); showNotification("Invoice rejected ✓"); }}
                            style={{ marginTop:10, fontSize:12, padding:"6px 14px", borderRadius:8, background:"#FEF2F2", border:"1px solid #DC262633", color:"#DC2626", cursor:"pointer" }}>✕ Not relevant — reject</button>
                        </>
                      ) : item.isDuplicate ? (
                ...
                return (
                  <div onClick={()=>setView("tax")} style={{ cursor:"pointer", background: dl.days<=14?"#FEF2F2":"#FFFBEB", border:`1px solid ${dl.color}55`, borderRadius:14, padding:"16px 20px", marginBottom:16, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10 }}
                    ...
                    <div>
                      ...
                      <div style={{ fontSize:12, color:"#6B7280", marginTop:3 }}>Pay or file at <a href={dl.url} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} style={{ color:dl.color }}>{dl.est?"irs.gov/payments":"irs.gov"} ↗</a> · click for your full tax picture</div>
                    </div>
