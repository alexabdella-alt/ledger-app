  ...
  return (
            <div style={{ maxWidth:680 }}>
              <div style={{ marginBottom:28 }}>
                <div style={{ fontSize:10, letterSpacing:3, color:"#86868F", marginBottom:8 }}>NEW ENTRY</div>
              ...
              {!uploadedFile && (
                <div style={{ display:"flex", gap:10, marginBottom:20 }}>
                  {[
                    ...
                    <button key={btn.label} onClick={btn.onClick} style={{ padding:"8px 16px", borderRadius:20, fontSize:12, background:btn.active?"#C7BFFF":"transparent", border:`1px solid ${btn.active?"#C7BFFF":"#262629"}`, color:btn.active?"#0C0C0E":"#86868F", cursor:btn.onClick?"pointer":"default", fontWeight:btn.active?600:400 }}>{btn.label}</button>
                  ))}
                </div>
              )}
              {!uploadedFile && (
                <div onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)}
                  ...
                  style={{ border:`2px dashed ${dragOver?"#C7BFFF":"#262629"}`, borderRadius:16, padding:"56px 32px", textAlign:"center", cursor:"pointer", background:dragOver?"#18181C":"#141416", transition:"all 0.2s", marginBottom:24 }}>
                  <div style={{ fontSize:38, marginBottom:14 }}>⬆</div>
                  <div style={{ fontSize:16, fontWeight:500, marginBottom:8 }}>Drop your invoice here</div>
                  <div style={{ fontSize:13, color:"#86868F" }}>PDF, JPG, PNG or WEBP · AI reads, extracts vendor & codes automatically</div>
              ...
              {uploadedFile && (
                <div style={{ background:"#141416", border:"1px solid #1C1C20", borderRadius:16, padding:28, marginBottom:20 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                      <div style={{ width:36, height:36, background:"#1C1C20", borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16 }}>{uploadedFile.mediaType==="application/pdf"?"📄":"🖼"}</div>
                      <div>
                        <div style={{ fontSize:14, fontWeight:500 }}>{uploadedFile.name}</div>
                        <div style={{ fontSize:12, color:isAILoading?"#C7BFFF":"#10B981" }}>
                    ...
                    <button onClick={()=>{setUploadedFile(null);setAiSuggestion(null);setForm({vendor:"",description:"",amount:"",date:"",type:"expense",notes:"",project:"General"});}}
                      style={{ background:"none", border:"none", color:"#86868F", cursor:"pointer", fontSize:20 }}>×</button>
                  </div>
                  {isAILoading && (
                    <div style={{ marginBottom:20 }}>
                      <div style={{ height:3, background:"#1C1C20", borderRadius:2, overflow:"hidden" }}>
                        <div style={{ height:"100%", background:"linear-gradient(90deg,#6D5EF6,#C7BFFF)", borderRadius:2, width:aiStep==="coding"?"85%":"45%", transition:"width 1.2s ease", animation:"pulse 2s ease-in-out infinite" }} />
                      </div>
                    </div>
                  )}
                  {!isAILoading && (form.vendor||form.amount) && (
                    <div>
                      <div style={{ fontSize:11, color:"#86868F", letterSpacing:2, marginBottom:16 }}>EXTRACTED FIELDS — REVIEW & EDIT</div>
                      <div style={{ marginBottom:16, background:"#0A0A0C", border:`1px solid ${form.vendor?"#33333A":"#EF4444"}`, borderRadius:10, padding:14 }}>
                        <label style={{ ...labelStyle, color:"#C7BFFF" }}>VENDOR NAME <span style={{ color:"#EF4444" }}>*</span></label>
                        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                          ...
                          <input value={form.vendor} onChange={e=>handleFormChange("vendor",e.target.value)} placeholder="Vendor name — required" style={{ ...inputStyle, border:!form.vendor?"1px solid #EF4444":"1px solid #33333A", background:"#0C0C0E", fontWeight:500, fontSize:14 }} />
                        </div>
                      ...
                      <div style={{ marginBottom:14 }}>
                        <label style={labelStyle}>INVOICE NUMBER <span style={{ color:"#86868F", fontWeight:400, fontSize:10 }}>(optional — used for duplicate detection)</span></label>
              ...
              {aiSuggestion && (
                <div style={{ background:"#0A0A0C", border:"1px solid #33333A", borderRadius:14, padding:24, marginBottom:20 }}>
                  <div style={{ fontSize:11, color:"#C7BFFF", letterSpacing:2, marginBottom:16 }}>✦ AI GL CODING</div>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
                    <div style={{ background:"#141416", borderRadius:10, padding:14 }}>
                      <div style={{ fontSize:11, color:"#86868F", marginBottom:6 }}>PRIMARY ACCOUNT</div>
                      <div style={{ fontSize:15, fontWeight:600, color:"#C7BFFF" }}>{aiSuggestion.gl_code}</div>
                      <div style={{ fontSize:13, color:"#F2F2F4", marginTop:2 }}>{aiSuggestion.gl_name}</div>
                      <div style={{ fontSize:11, color:"#86868F", marginTop:4 }}>{aiSuggestion.debit_credit?.toUpperCase()}</div>
                    </div>
                    <div style={{ background:"#141416", borderRadius:10, padding:14 }}>
                      <div style={{ fontSize:11, color:"#86868F", marginBottom:6 }}>OFFSET ACCOUNT</div>
                      <div style={{ fontSize:15, fontWeight:600, color:"#9A9AA2" }}>{aiSuggestion.secondary_gl_code}</div>
                      <div style={{ fontSize:13, color:"#9A9AA2", marginTop:2 }}>{aiSuggestion.secondary_gl_name}</div>
                    </div>
                  </div>
                  <div style={{ fontSize:12, color:"#9A9AA2", marginBottom:14, lineHeight:1.7 }}>{aiSuggestion.reasoning}</div>
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    <div style={{ height:5, flex:1, background:"#1C1C20", borderRadius:3 }}>
