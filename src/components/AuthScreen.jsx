...
function AuthScreen({ onAuth }) {
  ...
  const label = { fontSize:11, color:"#6B7280", marginBottom:6, letterSpacing:1, fontWeight:500 };
  const input = { width:"100%", boxSizing:"border-box", background:"#F3F4F6", border:"1px solid #D1D5DB", borderRadius:11, padding:"12px 14px", color:"#111827", fontSize:14, outline:"none", marginBottom:14, transition:"border-color .2s, box-shadow .2s", fontFamily:"'DM Sans', sans-serif" };
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", flexWrap:"wrap", fontFamily:"'DM Sans', system-ui, sans-serif", color:"#111827", position:"relative", overflow:"hidden" }}>
      {/* ── Left: brand hero ── */}
      <div style={{ flex:"1 1 460px", minWidth:0, padding:"clamp(40px, 6vw, 96px)", display:"flex", flexDirection:"column", justifyContent:"space-between", gap:48, position:"relative" }}>
        <div className="sc-rise" style={{ display:"flex", alignItems:"center", gap:14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:11, color:"#6B7280", letterSpacing:2, marginTop:2 }}>AUTONOMOUS FINANCE</div>
          </div>
        </div>
        ...
        <div style={{ maxWidth:540 }}>
          ...
          <p className="sc-rise-2" style={{ fontSize:16, lineHeight:1.65, color:"#6B7280", margin:"0 0 40px", maxWidth:460 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
            {features.map(([t, d], i) => (
              <div key={t} className={`sc-rise-${i+2}`} style={{ display:"flex", gap:14, alignItems:"flex-start" }}>
                <div style={{ width:30, height:30, borderRadius:9, flexShrink:0, marginTop:2, background:"linear-gradient(135deg, rgba(139,123,255,.18), rgba(109,94,246,.08))", border:"1px solid #D1D5DB", display:"flex", alignItems:"center", justifyContent:"center", color:"#818CF8", fontSize:13 }}>✦</div>
                <div>
                  <div style={{ fontSize:14, fontWeight:600, marginBottom:3 }}>{t}</div>
                  <div style={{ fontSize:13, color:"#6B7280", lineHeight:1.55, maxWidth:420 }}>{d}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="sc-fade" style={{ fontSize:12, color:"#9CA3AF", letterSpacing:0.3 }}>
          Tenant-isolated (RLS) · ASC 842 compliant · Full audit trail
        </div>
      </div>

      {/* ── Right: auth card ── */}
      <div style={{ flex:"1 1 380px", display:"flex", alignItems:"center", justifyContent:"center", padding:"48px 32px", background:"#FFFFFF", borderLeft:"1px solid #E5E7EB", boxShadow:"-1px 0 0 #E5E7EB" }}>
        <div className="sc-scale" style={{ width:"100%", maxWidth:380 }}>
          ...
          {error && <div style={{ background:"#FEF2F2", border:"1px solid #DC262633", borderRadius:10, padding:"10px 14px", fontSize:13, color:"#DC2626", marginBottom:14 }}>{error}</div>}
          {message && <div style={{ background:"#ECFDF5", border:"1px solid #05966933", borderRadius:10, padding:"10px 14px", fontSize:13, color:"#059669", marginBottom:14 }}>{message}</div>}
          ...
          <div style={{ textAlign:"center", marginTop:22, fontSize:13, color:"#6B7280" }}>
            {mode === "login"
