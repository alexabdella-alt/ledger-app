...
function ERP({ session, currentCompany, companies, onSwitchCompany, setCurrentCompany, setCompanies, onNewCompany, onSignOut, supabase, persistedView, onViewChange }) {
  ...
  // + sign-off (O50), run through the SAME evaluateSignOff gate so the owner panel can never
  // disagree with the CPA's ReviewView. Pure + memoized off the live trust sources.
  const ownerTrust = useMemo(() => ownerTrustState({
    ...
    unknownDocs,
    reviewedThrough,
  }), [controlTotals, invoices, intakeRows, unknownDocs, reviewedThrough]);
  ...
  const signOffPeriod = async (period, note = null) => {
    ...
    const evalr = evaluateSignOff({
      controlTotals,
      openConfidenceFlags: flagsForReview(),
      droppedDocs: dropped,
      unknownDocs: unknownDocs || [],
    });
    if (!evalr.ok) return { ok: false, blockers: evalr.blockers };
