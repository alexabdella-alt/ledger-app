...
import { payrollHoldDetail } from "./lib/waitingOnYou";
...
function ERP({ session, currentCompany, companies, onSwitchCompany, setCurrentCompany, setCompanies, onNewCompany, onSignOut, supabase, persistedView, onViewChange }) {
  ...
  const reloadHeldIntake = async (intakeId) => {
    ...
    if (!row.document_id) return { ok: false, error: "no stored file for this register — it predates durable intake; drop the file again" };
    ...
    logAudit("payroll_register_reloaded", `Re-reading ${row.filename || "a payroll register"} from its stored file so it can be reviewed`, null, { intake_id: String(row.id), document_id: String(row.document_id) });
    return { ok: true };
  };
