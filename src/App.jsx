...
function ERP({ session, currentCompany, companies, onSwitchCompany, setCurrentCompany, setCompanies, onNewCompany, onSignOut, supabase, persistedView, onViewChange }) {
  ...
  const setVendor1099 = async (contact, next) => {
    if (!currentCompany?.id || !contact?.id) return { ok: false };
    const want = !!next;
    const before = !!contact.is1099;
    setContacts(prev => prev.map(c => (c.id === contact.id ? { ...c, is1099: want } : c)));
    const res = await checkedRowUpdate({
      supabase, table: "contacts", id: contact.id, companyId: currentCompany.id,
      patch: { is_1099: want }, label: "contacts:set-1099",
    });
    if (!res.ok) {
      // Put the badge back rather than leaving it showing a state the books do not hold.
      setContacts(prev => prev.map(c => (c.id === contact.id ? { ...c, is1099: before } : c)));
      logAudit("1099_flag_write_failed", `Couldn't save the 1099 setting for ${contact.name} (${res.reason})`, null, { contact_id: String(contact.id), intended: want, reason: res.reason });
      showNotification(`Couldn't save the 1099 setting for ${contact.name} — it's unchanged. Try again.`, "error");
      return { ok: false };
    }
    logAudit("1099_flagged", `${contact.name} ${want ? "marked as needing" : "no longer needs"} a 1099`, { is_1099: before }, { is_1099: want });
    return { ok: true };
  };
