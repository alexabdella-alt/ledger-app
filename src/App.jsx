...
import { planContractEntries } from "./lib/contractEntries";
...
import { contactDbId, isDbId } from "./lib/contactIds";
import { settleIntakeAfterAnswers, ANSWER_OUTCOME } from "./lib/clarificationSettle";
...
import { bookableAccounts, disableAccountBlocker } from "./lib/chart";
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--sc-bg)", display:"flex", alignItems:"center", justifyContent:"center", padding:24, fontFamily:"'DM Sans', system-ui, sans-serif" }}>
      <div style={{ background:"var(--sc-surface)", border:"1px solid var(--sc-border)", borderRadius:16, boxShadow:"0 8px 28px rgba(17,24,39,0.10)", padding:32, maxWidth:480, width:"100%", textAlign:"center" }}>
        ...
        <p style={{ fontSize:14, color:"var(--sc-text-2)", lineHeight:1.6, margin:"0 0 20px" }}>{isSentryEnabled() ? "Our team has been notified. Refreshing usually fixes it — nothing you've recorded is lost; it's saved on the server." : "Refreshing usually fixes it — nothing you've recorded is lost; it's saved on the server."}</p>
...
function ERP({ session, currentCompany, companies, onSwitchCompany, setCurrentCompany, setCompanies, onNewCompany, onSignOut, supabase, persistedView, onViewChange }) {
  ...
  // ★★ THIS WAS `userRole === "member"`, AND NO USER CAN EVER HAVE THAT ROLE.
  // `company_users_role_check` allows exactly `owner | admin | accountant | viewer`. So
  // `isMember` was permanently FALSE, and every read-only protection built on it — the
  // delete/void controls on the transaction panel, the AI's mutating actions, the settings
  // sub-tabs — **has never once applied.** The database still refused a viewer's writes
  // (RLS role gates, C199), so nothing was corrupted; what a viewer got was a screen full
  // of buttons that would fail, which is the "refusing on click" anti-pattern by accident.
  //
  // The app's "member" and the database's "viewer" were the same idea under two names, and
  // only one of them existed. `isMember` is kept as an alias because callers read it, and
  // is now TRUE for the role that actually exists.
  const isViewer = userRole === "viewer";
  const isMember = isViewer;
  ...
  const clearTransientUploadState = () => {
    setUploadedFile(null); setUploadQueue([]); 
  ...
  // C472 — THE CATEGORIES A PERSON (OR THE MODEL) MAY PICK FOR NEW WORK. "Disable" on the
  // Categories screen wrote `active: false` and nothing read it: every picker and all seven
  // AI chart slots still offered the account, so the control's only effect was a greyer
  // row. Historical rows keep resolving through the FULL chart (a name is still a name);
  // this is the list for choosing.
  const BOOKABLE_ACCOUNTS = React.useMemo(() => bookableAccounts(CHART_OF_ACCOUNTS), [CHART_OF_ACCOUNTS]);
  ...
  const persistAccountEdit = async (account, updates) => {
    ...
    // C472 — "Disable" is refused, with the reason, where it would change nothing that
    // matters or strand a rule pointing at the account.
    if (updates.active === false) {
      const why = disableAccountBlocker(account, { rules, recurring, bankAccounts });
      if (why) { showNotification(why, "error"); return false; }
    }
  ...
  // O113b — how much reading is left this hour, in documents. The proxy has been returning
  // it since O113a and nothing read it, so the ceiling was only discoverable by hitting it.
  ...
  const loadStatementExceptions = async (companyId) => {
    ...
    try {
      ...
      // ★ O98 — RECORD THAT THE CHECK DIDN'T RUN. This swallowed into `[]`, which the
      // Review queue reads as "no statement exceptions" — so a failed query could render
      // the same green "All clear — nothing needs review" as a genuinely clean month.
  ...
  const [statementExceptionsLoadFailed, setStatementExceptionsLoadFailed] = useState(false);   // O98
  ...
  const [aiDegraded, setAiDegraded] = useState(null);   // { kind, at } — document reading is paused
  const [drainStatus, setDrainStatus] = useState(null); // { plan, startedAt } — last drain census, for the owner copy
  // O82 — the email channel (C351). `mailChannel` is loadMailChannel's whole result so a
  // screen can tell "we could not ask" from "no channel" (O98); `heldInbound` is the messages
  // the sender wall refused, awaiting a person.
  const [mailChannel, setMailChannel] = useState(null);
  const [heldInbound, setHeldInbound] = useState([]);
  ...
  const resetCompanyState = () => {
    ...
    setMailChannel(null); setHeldInbound([]);
    ...
    setReconciliations([]); setOpeningBalances([]);
    ...
    setSelectedInvoice(null); setReturnTo(null); 
    setAiSuggestion(null); setAiStep(null);
    setSelectedContract(null); setContractView("list");
    
    setDeleteConfirm(null); setDocsPreview(null);
    ...
    setChatHistoryView(false);

    // In-progress wizards / processing flags / drag states
    setIsAILoading(false); setUploadedFile(null); 
    ...
    // C410 — the sign-off / intake effect loads these per company and never cleared them, so
    // a company whose read failed (or had not resolved yet) showed the PREVIOUS company's
  ...
  // C409 — the order of the settled reads below, by table, so a failure is named.
  const LOAD_TABLES = ["contacts", "vendor_rules", "companies", "opening_balances", "bank_accounts", "recurring_transactions", "ar_invoices", "audit_log", "documents", "reconciliations", "upload_log", "unknown_documents"];
  const loadAllData = async () => {
    ...
    try {
      ...
      const [
        contactsRes, rulesRes, coRes, obRes, banksRes, recRes, arRes, auditRes, docsRes, reconRes, ulRes, unkRes,
      ] = settled.map((r) => (r.status === "fulfilled" ? (r.value || {}) : {}));
      // C409 — and the failures are RECORDED, so the screen behind each table can say it could
      // not check rather than rendering its empty state over a read that never ran.
      const failures = settledFailures(settled, LOAD_TABLES);
      setLoadFailures(failures);
      for (const [t, msg] of Object.entries(failures)) console.warn(`[loadAllData] ${t} did not load:`, msg);
      ...
      if (contactsData) {
        setContacts(contactsData.map(c => ({
          ...
          // `...c` carries the snake_case name only, so a flag genuinely set in the
          // database read back as `undefined` and rendered "Not flagged" — and the count
          // of vendors needing a 1099 was structurally zero. Mapped here, once, where the
          // rest of the row's aliases already live.
          is1099: !!c.is_1099,
      ...
      const helpers = await Promise.allSettled([
        ...
        loadFiledDeadlines(cid),        // C388 — deadlines already marked filed
      ...
      // C364 — the catch-all's review records. A failed read leaves the in-session list alone
      // rather than emptying it (O98: "could not ask" is not "there are none").
      if (Array.isArray(unkRes.data)) setUnknownDocs(unkRes.data.map(unknownDocFromRow).filter(Boolean));
      else if (unkRes.error) console.warn("[loadAllData] unknown_documents read failed:", unkRes.error.message || unkRes.error);
  ...
  // ── "REDO OPENING SETUP" (§12 deferred) ──────────────────────────────────────
  // The cutoff locks once opening balances are posted, which is right — but it left no way
  // back from a starting position set up wrong, and that is a plausible week-one mistake
  // with permanent consequences.
  //
  // ★★★ IT UNLOCKS, IT DOES NOT DELETE. Reversing the opening entry now would leave the
  // books with NO starting position in between — every report in that window wrong in a new
  // way, and permanently so if the person walks away mid-repair. `postOpeningBalances`
  // already supersedes the previous entry when a new one is posted (tested, and hardened
  // today), so the old position stays live and coherent until a better one replaces it.
  const redoOpeningPlan = () => planOpeningRedo({
    openingPosted, cutoffDate, canEdit: !!(isOwner || isAdmin),
    signedPeriodOf: (d) => { const p = signedPeriodForDate(d, signoffs); return p ? (signedMonthLabel(p) || p) : null; },
  });
  const redoOpeningSetup = async () => {
    const plan = redoOpeningPlan();
    if (!plan.ok) { showNotification(plan.message, "error"); return false; }
    // Unlock by clearing the POSTED flag on the opening rows — `openingPosted` is derived
    // from them, and the cutoff guard reads that. The journal entry is untouched.
    const ids = (openingBalances || []).filter(b => b.posted && b.id).map(b => b.id);
    if (ids.length) {
      const r = await checkedIdsUpdate({
        supabase, table: "opening_balances", ids, companyId: currentCompany?.id,
        patch: { posted: false }, label: "redoOpeningSetup",
      });
      if (!r.ok) { showNotification("Couldn't unlock your start date — nothing was changed. Please try again.", "error"); return false; }
    }
    setOpeningBalances(prev => (prev || []).map(b => b.posted ? { ...b, posted: false } : b));
    logAudit("opening_setup_reopened", redoAuditDetail({ cutoffDate }), null, { cutoff: cutoffDate, rows: ids.length });
    showNotification("Start date unlocked — enter your starting balances again when you're ready. The current ones stay in your books until you replace them.");
    return true;
  };
  ...
  // C331 — a `cashFromBanks` helper (Σ bank_accounts.current_balance) was exposed on the
  // context and read by NOTHING. Removed rather than left: it computed exactly the figure
  // §12 says must never be shown as cash on hand (the bank's number is the reconciliation
  // TARGET; cash is GL-derived, `glCash`), so the next person reaching for a cash total
  // would have found a wired-looking function that produces the wrong one.
  ...
  const dismissAnomaly = async (id, reason, evidenceDocIds = []) => {
    const cid = currentCompany?.id;
    ...
    // Evidence is OPTIONAL and stays optional (see `anomalyNotes.js`) — a hard requirement
    // produces attachments chosen for being nearest, which is worse than none because it
  ...
  // ── THE OWNER'S HALF OF THE CARD — context WITHOUT a clear verb ───────────────
  // The owner usually knows why a flagged charge is fine and had no way to say so; the
  // card's only verb is one they are permanently not allowed to use. A comment never
  // touches `status`, which is exactly what keeps the separation intact: they inform the
  // judgement, the reviewer makes it. Append-only in the database (`080` grants SELECT and
  // INSERT and nothing else) — an editable comment is not evidence of anything.
  const [anomalyComments, setAnomalyComments] = useState([]);
  // C388 — which tax deadlines this company has marked filed (tax_settings.filed_deadlines,
  // current tax year). Read once at load so Home's list and the bell can skip a deadline the
  // person already dealt with; TaxView writes the row and pushes the new map here.
  const [filedDeadlines, setFiledDeadlinesState] = useState({});
  // C409 — which secondary tables failed to load for this company (`{ table: message }`), so a
  // records screen can say "we couldn't check" instead of "there are none".
  const [loadFailures, setLoadFailures] = useState({});
  const filedDeadlinesRef = useRef({});
  const setFiledDeadlines = (m) => { const v = (m && typeof m === "object" && !Array.isArray(m)) ? m : {}; filedDeadlinesRef.current = v; setFiledDeadlinesState(v); };
  const loadFiledDeadlines = async (cid) => {
    if (!cid) return;
    try {
  ...
  const loadAnomalyComments = async (cid) => {
    if (!cid) return;
    try {
  ...
  const addAnomalyComment = async (anomalyId, body) => {
    const cid = currentCompany?.id;
    const v = validateComment(body);
    if (!v.ok) return { ok: false, error: v.error };
    if (!cid || !anomalyId) return { ok: false, error: "missing company/anomaly" };
    try {
      // `.select()` for the same reason every write here has it: without it a write that
      // stored nothing is indistinguishable from one that worked (C192).
      const { data, error } = await supabase.from("anomaly_comments")
        .insert({ company_id: cid, anomaly_id: anomalyId, body: v.text, author_id: session?.user?.id || null, author_name: session?.user?.email || null })
        .select("id");
      if (error) return { ok: false, error: error.message };
      if (!data || !data.length) return { ok: false, error: "The note didn't save — please try again." };
      logAudit("anomaly_commented", `Added context to a flagged item`, null, { anomaly_id: anomalyId });
      await loadAnomalyComments(cid);
      return { ok: true };
    } catch (e) { return { ok: false, error: e?.message || String(e) }; }
  };
  ...
  const maybeGenerateMonthlyReport = async () => {
    ...
    try {
      ...
      const payload = buildMonthlyReport(target, {
        ...
        apCode: rc("accounts_payable"), arCode: rc("accounts_receivable"),   // one definition of "money you owe" / "owed to you" (§12)
  ...
  // ── TIER 1 #6 — GIVE THE CHART THE ACCOUNTS THIS BUSINESS ACTUALLY NEEDS ─────
  // Runs the moment we LEARN the business type, which is here and not at company creation:
  // setup asks only for a name, so the generic chart is necessarily already seeded. That
  // makes an overlay the correct shape rather than a workaround.
  //
  // ★★ THE SEED RPC IS NOT TOUCHED. §6: the repo holds five definitions of
  // `seed_company_accounts` and the highest-numbered was never applied and could not be, so
  // any `create or replace` must start from `pg_get_functiondef` against live. This adds
  // accounts through the same audited path a human uses — no migration, no RPC change.
  //
  // ★ WHY IT MATTERS BEYOND TIDINESS: the categoriser is CONSTRAINED TO THE CHART IT IS
  // GIVEN. A restaurant with no food-cost account does not get a blank — it gets a wrong
  // account, which is how a Toast merchant fee reached `6500 Technology & Software` and
  // `Alamo Ice & Beverage` reached `7100 Miscellaneous`.
  const applyCoaTemplate = async (businessType) => {
    const cid = currentCompany?.id;
    if (!cid || !businessType) return { ok: false };
    try {
      const { data: existing } = await supabase.from("accounts")
        .select("id, code, name, active").eq("company_id", cid);
      // Codes that already carry transactions — derived from the LEDGER, so an account with
      // history can never be deactivated by a template.
      const usedCodes = [...new Set((invoicesRef.current || [])
        .flatMap(i => [i.gl_code, i.secondary_gl_code]).filter(Boolean).map(String))];

      const plan = planCoaTemplate(businessType, existing || [], usedCodes);
      if (!plan.add.length && !plan.hide.length) return { ok: true, plan };

      // The add is audited immediately after ITS write, and the hide after ITS loop — two
      // events, each describing only what actually happened (§9). One `buildAccountInsert`
      // site: the pre-070 fallback strips `origin` from the rows already built rather than
      // building them again, so the O108 guard has one door to account for, not two.
      const failCopy = "Couldn't finish setting up your chart of accounts — you can add accounts in Settings.";
      if (plan.add.length) {
        const rows = plan.add.map(a => buildAccountInsert({ ...a, companyId: cid, origin: "seed" }));
        let { error } = await supabase.from("accounts").insert(rows);
        if (error && /origin|column/i.test(error.message || "")) {
          ({ error } = await supabase.from("accounts").insert(rows.map(({ origin, ...r }) => r)));
        }
        if (error) { console.error("[coa-template]", error.message); showNotification(failCopy, "error"); return { ok: false, error: error.message }; }
        logAudit("coa_template_applied",
          `${businessType}: added ${plan.add.length} account(s) — ${plan.add.map(a => a.code).join(", ")}`,
          null, { business_type: businessType, added: plan.add.map(a => a.code), skipped: plan.skipped });
      }
      const hidden = [];
      for (const code of plan.hide) {
        const row = (existing || []).find(a => String(a.code) === String(code));
        if (!row) continue;
        const r = await checkedRowUpdate({ supabase, table: "accounts", id: row.id, companyId: cid, patch: { active: false }, label: "coa-template:hide" });
        if (r.ok) hidden.push(code);
      }
      if (hidden.length) {
        logAudit("coa_template_applied", `${businessType}: hid ${hidden.length} unused account(s) — ${hidden.join(", ")}`,
          null, { business_type: businessType, hidden });
      }
      try { await reloadAccounts(); } catch {}
      const copy = coaTemplateCopy({ ...plan, hide: hidden });
      if (copy) showNotification(copy);
      return { ok: true, plan };
    } catch (e) {
      console.error("[coa-template]", e?.message || e);
  ...
  // ── O130 — THE ONE "REMOVE THIS" ACTION THE OWNER SEES ───────────────────────
  // Replaces the Void/Delete pair. `planEntryRemoval` decides which mechanism is correct
  // from the ONE input that actually determines it — has the month been signed off — and
  // returns the plain-language sentence alongside the decision, so a caller cannot render
  // "Deleted" over a correction or the reverse (§9).
  //
  // Both mechanisms are unchanged and still tested; what changes is that the OWNER no
  // longer picks between them. "Void" and "reversal" are bookkeeper words, and being wrong
  // about which one to press is how one invoice ended up reversed three times (O123).
  const removeEntry = async (invoice, byAI = false) => {
    ...
    if (plan.mode === REMOVAL.CORRECT) {
      ...
      try { await loadAllData(); } catch {}
      showNotification(plan.done, "success");
      return { ok: true, mode: plan.mode, id: revId };
    }
    const ids = await softDeleteInvoices([invoice], byAI);   // owns its own Undo toast
    return { ok: Array.isArray(ids) && ids.length > 0, mode: plan.mode, ids };
  };
  // The confirmation text for a given entry — the SAME planner, so the modal can never
  // promise one outcome while the action performs the other.
  const removalPlanFor = (invoice) => planEntryRemoval(invoice, signoffs, { monthLabel: signedMonthLabel });
  ...
  // ★ C360 — THE RECURRING SCREEN WRITES THROUGH THE SAME PATH THE CHAT DOES. Its "+ Add"
  // was a setState and its "Post now" put an entry in React state, wrote an audit row saying
  // it had posted, and never called `bookToDb` — the C288 shape, on a screen a client can
  // reach through Settings. One writer for a rule (`persistChatRecurring`, verified insert),
  // and the run is RECORDED on the row so a reload cannot offer the same month again.
  const createRecurring = (args) => persistChatRecurring(args);
  // ★ C364 — the posted/dismissed markers on an unknown-document record are written back.
  // An in-session-only record (no durable document) returns ok — there is no row to keep in
  // step, and the marker is honest for this sitting.
  const persistUnknownDocPatch = async (doc, patch) => {
    if (!isDbUnknownDocId(doc?.db_id)) return { ok: true, inSession: true };
    return checkedRowUpdate({ supabase, table: "unknown_documents", id: doc.db_id, companyId: currentCompany?.id, patch, label: "unknownDocPatch" });
  };
  ...
  // and a "deleted" charge kept posting. Both write through the chat's verified helpers.
  const setRecurringActive = async (r, active) => {
    if (r?.id == null || typeof r.id === "number") return { ok: false, error: "this recurring charge isn't saved yet" };
    const res = await updateVerified(supabase, "recurring_transactions", r.id, { active: !!active });
    if (res.ok) setRecurring(prev => prev.map(x => x.id === r.id ? { ...x, active: !!active } : x));
    return res;
  };
  const removeRecurring = async (r) => {
    if (r?.id == null || typeof r.id === "number") { setRecurring(prev => prev.filter(x => x.id !== r?.id)); return { ok: true, local: true }; }
    const res = await deleteVerified(supabase, "recurring_transactions", { id: r.id, company_id: currentCompany?.id });
    if (res.ok) { setRecurring(prev => prev.filter(x => x.id !== r.id)); logAudit("recurring_deleted", `Deleted recurring charge: ${r.name}`, { name: r.name, amount: r.amount }, null); }
    return res;
  };
  // C398 — the bank-accounts × on Settings filtered local state; the row was back on reload
  // (the load reads active=true, so "removed" is `active: false`, checked).
  const removeBankAccount = async (ba) => {
    if (!isDbId(ba?.id)) { setBankAccounts(prev => prev.filter(b => b.id !== ba?.id)); return { ok: true, local: true }; }
    const r = await checkedRowUpdate({ supabase, table: "bank_accounts", id: ba.id, companyId: currentCompany?.id, patch: { active: false }, label: "removeBankAccount" });
    if (r.ok) { setBankAccounts(prev => prev.filter(b => b.id !== ba.id)); logAudit("bank_account_removed", `Bank account removed: ${ba.name || ba.id}`, { name: ba.name, gl_code: ba.gl_code }, null); }
    return r;
  };
  ...
  // ── TIER 1 #10 — SETTING A VENDOR'S 1099 FLAG, FOR REAL ─────────────────────
  // ★★ THE TOGGLE NEVER WROTE TO THE DATABASE. It called `setContacts` and `logAudit` and
  // stopped — so flagging a vendor for a 1099 LOOKED done (the badge flipped, an audit row
  // appeared) and was gone on the next reload. The §9 rule this breaks is the harshest one
  // in the file: an action whose effect is invisible will be repeated — except here the
  // effect was visible and had simply not happened.
  //
  // ★ WHY IT MATTERS MORE THAN A NORMAL LOST WRITE: this flag decides who gets a 1099 filed
  // under the accountant's name at year end. A silent revert means a vendor the CPA
  // deliberately marked comes back unmarked, and nobody is told.
  //
  // ▶ DELIBERATELY NOT DERIVED. Eligibility (services vs goods, the payee's entity type,
  // the $600 threshold) is TIER 3 and is NOT computed here. The flag stays OFF by default —
  // which the database already enforces (`is_1099 boolean default false not null`) — and
  // goes on only when a human sets it. An automatic guess would be exactly the wrong thing
  // to be confidently wrong about.
  // ★★★ `setVendor1099` REMOVED (C316) — IT WROTE A FIELD NOBODY READS. Since C256 made
  // 1099 eligibility a DERIVATION, `contacts.is_1099` is consulted by nothing: the Tax
  // page, `form1099.js` and the AI context all read `is_1099_exempt` and `business_type`
  // instead. So the badge it drove was a control that changed nothing — and could show
  // "1099 ✓" on a vendor the Tax page correctly reports as goods-only. The human's real
  // levers into the derivation are the two fields that ARE read, and both already have UI.

  // ── O111 — THE ALIAS INDEX ───────────────────────────────────────────────────
  // Built from `contacts`, so it changes when a person adds or removes an alias and never
  // otherwise. `applyAlias` is a Map lookup by design: no scoring, no fuzzy match, no "did
  // you mean". A suggestion engine is O106 — a different feature with a different risk
  // profile — and this one only ever applies a merge a human has already asserted.
  ...
  const persistContact = async (contact) => {
    ...
    try {
      const base = {
        ...
        // O111 — other names this supplier appears under. Asserted by a person; never
        // inferred. Dropped by the pre-migration fallback below if `082` isn't applied yet.
  ...
  const applyGaapAnswer = async (item, opt) => {
    ...
    // C367 — each branch reports what it landed so the document's intake row can settle.
    if (opt.prepaidMonths) { const capId = await bookPrepaid(inv, opt.prepaidMonths, opt); if (capId) settleClarification(item, { kind: ANSWER_OUTCOME.BOOKED, jeId: capId }); return; }
    if (opt.deferredRevenueReceipt) { const drId = await bookDeferredRevenueReceipt(inv, opt); if (drId) settleClarification(item, { kind: ANSWER_OUTCOME.BOOKED, jeId: drId }); return; }
    if (opt.bookAsIs) {
  ...
  // O129 — THE "DEPRECIATION IS DUE" SIGNAL IS GONE, AND IT HAD BEEN DEAD FOR A WHILE.
  // `depreciationDueInfo` existed to feed a dashboard nudge ("N months due · Run
  // depreciation now"). That nudge was REMOVED when depreciation became automatic — a
  // deterministic accounting task is not something to prompt a human about — but the
  // signal behind it was left running: a Supabase query on EVERY company load, writing
  // state that exactly one file destructured and no file read.
  //
  // ★ Same class as O96, with a bill attached: dead plumbing that merely reads oddly costs
  // nothing, but this one made a database round-trip per load, forever, to answer a
  ...
  // ▶ NO CALLER SINCE 2026-08-29 (O129). Its only door was the Reports "attach to existing
  // asset" form, removed at the operator's call — it asked for a raw `journal_entries.id`,
  // typed by hand, on the tab where you go to read your statements. The LOGIC is sound and
  // repairs a real (rare) case the normal capitalize flow misses, so it is kept rather
  // than deleted on my own initiative — but it is kept OUT of `erpCtx`, and named here as
  // unreachable, because the thing O96 punished was dead plumbing that LOOKED wired.
  // Tracked on the ledger as a decision to make, not as code that quietly rots.
  ...
  const markIntake = (intakeId, status, opts = {}) => {
    ...
    setIntakeStatus(supabase, intakeId, status, opts).then(res => {
      if (!res.ok) { console.warn("[document_intake] status update failed:", status, res?.error); return; }
      // ★ C367 — the in-session snapshot follows the write. `intakeRows` is loaded once per
      // company; a row re-read and answered in this sitting would otherwise still read HELD
      // here, and C365's card would offer the question again the moment its card closed.
      if (res.row) setIntakeRows(prev => (prev || []).some(r => String(r.id) === String(intakeId)) ? prev.map(r => String(r.id) === String(intakeId) ? res.row : r) : prev);
    }).catch(e => console.warn("[document_intake] markIntake error:", e));
  };
  // ★ C367 — WHEN A QUESTION IS ANSWERED, THE DOCUMENT'S ROW MOVES. The hold site records
  // (per upload item) which intake row is waiting and which entries the batch had already
  ...
  const settlePartialHold = (invoice, jeId) => {
    ...
    markIntake(hold.intakeId, INTAKE_STATUS.HELD, { journalEntryIds: hold.ids, detail: partialHoldDetail(hold.ids.length, hold.expected) });   // C390 — the second writer of the same sentence, found by the guard
  ...
  const settleClarification = (item, outcome) => {
    ...
    if (!hold || !hold.intakeId) return { ok: false, reason: "no held intake row for this card" };
    hold.outcomes.push(outcome);
    const remainingCards = (clarificationQueueRef.current || [])
      .filter(c => c && String(c.queueItemId ?? "") === key && !c.resolved && c.id !== item.id).length;
    const plan = settleIntakeAfterAnswers({ outcomes: hold.outcomes, existingIds: hold.existingIds, remainingCards });
    if (!plan.status) return { ok: true, settled: false };
    markIntake(hold.intakeId, plan.status, { journalEntryIds: plan.journalEntryIds, detail: plan.detail });
    delete clarificationHoldsRef.current[key];
    return { ok: true, settled: true, status: plan.status };
  };
  ...
  // ── O135 — REBUILD A HELD PAYROLL REGISTER FROM ITS STORED BYTES ─────────────
  // The parsed register is React state and dies with the tab; the intake row and the
  // stored file do not. Putting the bytes back through the normal pipeline — under the
  // ORIGINAL intake id, so no second arrival row — re-parses, re-runs the gate, and lands
  // the hold in-session where the override card can act on it. One AI call. The proper
  // end-state is a persisted register (a migration); this is the no-migration door.
  const reloadHeldIntake = async (intakeId) => {
    const row = (intakeRows || []).find(r => String(r.id) === String(intakeId));
    if (!row) return { ok: false, error: "that intake row is not loaded" };
    if (!row.document_id) return { ok: false, error: "no stored file for this document — it predates durable intake; drop the file again" };
    const got = await fetchStoredFile({ supabase, documentId: row.document_id });
    if (!got.ok) return { ok: false, error: got.error };
    enqueueDrainedItem({ intakeId: row.id, file: got.file, filename: row.filename || got.document?.name || "payroll" });
    logAudit("intake_reloaded", `Re-reading ${row.filename || "a document"} from its stored file so it can be reviewed (${row.detail || "held"})`, null, { intake_id: String(row.id), document_id: String(row.document_id) });
    return { ok: true };
  };

  // ── O102 — SHADOW MODE: THE LADDER COMPUTES, AND BOOKS NOTHING ──────────────
  // The signed spec's sequencing decision, in its own words: the ladder computes its verdict
  // ALONGSIDE confidence, recording what it WOULD have booked, for at least one full drive
  // before it books anything — *because C198·3a shipped a gate that could never fire and a
  // green suite could not tell.*
  //
  // ★★ THE RUNNER HAS EXISTED SINCE C201 AND WAS INVOKED BY NOTHING, which is the same state
  // as not existing: C195(7)'s lesson is that a mechanism nobody triggers is indistinguishable
  // from one with nothing to do. This is the trigger and the scoring, and NOTHING ELSE — no
  // booking path consults any of it.
  const [shadowResult, setShadowResult] = useState(null);
  const shadowBusyRef = useRef(false);
  const runShadowCalibration = async ({ from, to } = {}) => {
    if (!currentCompany?.id || shadowBusyRef.current) return null;
    if (!from || !to) return { ok: false, error: "a date range is required" };
    shadowBusyRef.current = true;
    setShadowResult({ running: true });
    try {
      // ★ TWO PASSES OVER THE SAME INPUT, ON PURPOSE. §4.1(3) makes a verdict that varies
      ...
      logAudit("shadow_calibration_run", `Shadow pass ${from}..${to}: ${report.scored} lines scored, verdict ${report.verdict}`, null,
        { from, to, verdict: report.verdict, scored: report.scored, counts: report.counts });
      return { ok: true, report };
    } catch (e) {
      // ★ A FAILED PASS IS REPORTED AS A FAILED PASS, never as an empty one. A partial write
      // makes the denominator wrong, and a wrong denominator is how a weak result reads as a
      // strong one (Amendment A §2) — so `runShadowPass` throws rather than returning short,
      // and this must not turn that into a quiet zero.
      console.error("[shadow] pass failed:", e?.message || e);
      setShadowResult({ error: e?.message || String(e) });
      return { ok: false, error: e?.message || String(e) };
    } finally { shadowBusyRef.current = false; }
  };

  // ── O82 — THE EMAIL CHANNEL, CLIENT SIDE (C351) ─────────────────────────────
  const refreshMailState = async () => {
    if (!currentCompany?.id) return;
    const cid = currentCompany.id;
  ...
  const allowMailSender = async (email) => {
    ...
    const r = await addAllowedSender(supabase, { companyId: currentCompany.id, current: mailChannel?.channel?.allowed_senders || [], email });
    if (r.ok) { logAudit("mail_sender_allowed", `${String(email).toLowerCase()} may now send documents by email`); await refreshMailState(); }
    else showNotification(r.error || "Couldn't add that address.", "error");
    return r;
  };
  const releaseHeldInbound = async (message) => {
    if (!currentCompany?.id) return { ok: false };
    const r = await releaseInboundMessage(supabase, { companyId: currentCompany.id, message, allowedSenders: mailChannel?.channel?.allowed_senders || [] });
    if (r.ok) {
  ...
  const sendChannelMail = async (args) => sendMailViaFunction({ companyId: currentCompany?.id, ...args });
  ...
  // C365 — documents held for a question whose card is not on screen (a reload dropped it).
  ...
  // C369 — documents a person has to deal with whose upload tile is gone (a reload).
  const heldUnreadable = useMemo(() => heldUnreadableRows(intakeRows, { uploadQueue }), [intakeRows, uploadQueue]);
  const heldPartial = useMemo(() => heldPartialRows(intakeRows, { uploadQueue }), [intakeRows, uploadQueue]);   // C390
  const ownerTrust = useMemo(() => {
    ...
    return ownerTrustState({
      ...
      // hired an accountant still has self-signed months in its history and the panel must
      // keep telling the truth about those.
      selfSigned: !!(signoffs || []).find(so => so && !so.revoked_at && so.period === reviewedThrough)?.self_attested,
      ...
      // C365 — a question that was asked in a previous sitting and never answered is still
      // open: its intake row says so. Counting only the in-session cards read green over a
  ...
  const signOffPeriodOnce = async (period, { note = null, override = null, acknowledged = false } = {}) => {
    ...
    // while nobody else can do it, and only with the acknowledgement recorded on the row.
    // The moment a real accountant joins, `canSelfAttest` goes false and the separation is
    // back without anybody changing a setting.
    const selfAttesting = !isReviewer && canSelfAttest({ role: userRole, hasAttester });
    if (!isReviewer && !selfAttesting) {
      return { ok: false, error: "only a reviewer (accountant/admin) can sign off — the account owner can't attest their own books" };
    }
    // ★ THE ACKNOWLEDGEMENT IS REQUIRED, NOT ASSUMED. Signing on someone's behalf without
    // their having read what they were signing is the thing this whole path exists to avoid,
    // and migration `085` refuses the write anyway — better to say so here than to hand back
    // a policy error the person cannot act on.
    if (selfAttesting && !acknowledged) {
      return { ok: false, error: "we need you to confirm you're signing off your own books before we can record it" };
    }
    ...
    const overrideRec = (!gate.ok && override && override.acknowledged)
      ...
      : null;
    const res = await persistSignoff(supabase, { companyId: currentCompany.id, period, signedBy: session.user.id, note, override: overrideRec, selfAttested: selfAttesting });
    if (res.ok) {
      setSignoffs(prev => [res.row, ...prev.filter(s => s.period !== period)]);
      // §9 — the audit sentence reads the path actually taken, not the one intended.
      logAudit(
        "period_signed_off",
        selfAttesting
          ? `Signed off through ${period} by the owner themselves (no accountant on this company)${overrideRec ? ` (override: ${overrideRec.reason || "no reason given"})` : ""}`
          : (overrideRec ? `Reviewed through ${period} (override: ${overrideRec.reason || "no reason given"})` : `Reviewed through ${period}`),
        null,
        { period, override: !!overrideRec, blockers: overrideRec?.blockers || null, self_attested: selfAttesting },
      );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setSignoffsLoadOk(!!so.ok);   // C410 — a failed read must not read as "nothing signed"
  ...
  const processUploadItem = async (item, file) => {
    try {
        ...
        if (docType === "invoice") {
          ...
          extractedList.forEach((extracted, idx) => {
            ...
            const arrival = planInvoiceArrival(invoice, invoices, {
              ...
              // O111 — a merge this company's owner has asserted. Without it Franklin Ave's
              // invoice and its bank payment stay two vendors and the card asks a question
              // whose answer the person already gave us once.
              aliasIndex,
            });
          ...
          if (needsClarification.length > 0) {
            markIntake(item.intake_id, INTAKE_STATUS.HELD, { journalEntryIds: entryLinks.ids, detail: CLARIFICATION_HOLD_DETAIL });   // C365 — the constant the reload card reads
            clarificationHoldsRef.current[String(item.id)] = { intakeId: item.intake_id, existingIds: entryLinks.ids || [], outcomes: [] };   // C367
          }
            ...
            if (entryLinks.complete) {
              ...
              markIntake(item.intake_id, INTAKE_STATUS.HELD, {
                ...
                detail: partialHoldDetail(entryLinks.ids.length, entryLinks.expected),   // C390 — one sentence, shared with the reader
              });
          ...
          else markIntake(item.intake_id, INTAKE_STATUS.HELD, { detail: NOTHING_EXTRACTED_DETAIL });   // C369
          ...
          // ★ C364 — the record is WRITTEN, not merely held. `unknown_documents` had the exact
          // shape and no writer; a reload emptied the Review section and lost every watch
          // trigger. Needs the durable document id (NOT NULL, correctly) — a file the durable
          // store could not keep stays an in-session record, and that is said in the row's
          // absence rather than papered over with a fake id.
          const unkRow = unknownDocRow(unknownRecord, { companyId: currentCompany.id, documentId: durableDocId });
          if (unkRow) {
            const ins = await insertVerified(supabase, "unknown_documents", unkRow);
            if (ins.ok) unknownRecord = { ...unknownRecord, id: ins.row.id, db_id: ins.row.id, document_id: durableDocId };
            else logAudit("unknown_doc_persist_failed", `Couldn't save the review record for ${item.name} — it will be gone after a reload: ${ins.error}`, null, { file: item.name, document_id: durableDocId });
          }
      ...
      // ★ WHEN THE FAILURE CAME FROM THE AI CALL, IT ALREADY KNOWS WHICH FAILURE IT WAS.
      // `okAIResponse` attaches the classification, so the owner reads a sentence about
      // what actually happened — "we've hit our own hourly limit, which clears in about
      // 20 minutes" — instead of `AI service error (429 Too Many Requests)`. Four very
      // different situations used to arrive as that one string.
      //
      // ★★ AND IT MAY OVERRIDE THE TRANSIENT/PERMANENT CALL. Out-of-credit and bad
      // credentials look transient by status code and are NOT: no amount of retrying
      // clears them, so retrying forever would hide a thing only a human can fix.
      const ai = e && e.aiFailure ? e.aiFailure : null;
      const transient = ai ? ai.retryable : kind === FAILURE_KIND.TRANSIENT;
      const userMsg = ai
        ? ai.owner
        : (transient ? `${errMsg} — we'll keep trying; nothing is lost` : `${errMsg} — try again`);
      if (ai && ai.operator) console.error("[ai]", ai.kind, "—", ai.operator);
      // Degraded mode is a claim about the FEATURE, so only the two failures that actually
      // stop the feature may raise it — a single unreadable file never should.
      if (ai && isDegradedMode(ai.kind)) setAiDegraded({ kind: ai.kind, at: new Date().toISOString() });
      ...
      markIntake(
        ...
        // C369 — a PERMANENT hold carries the owner's sentence behind one prefix the Home card
        // reads; the operator detail is in the console line above. A transient failure keeps
        // the raw message: the drain retries it and nobody is asked to act.
        { detail: transient ? (ai ? `${ai.kind}: ${ai.operator}` : errMsg) : `${UNREADABLE_HOLD_PREFIX}${ai ? ai.owner : errMsg}` },
      );
    } finally {
      // Every processed file has just made AI calls, so this is the freshest the figure ever
      // gets — and it is exactly when someone is watching a queue drain.
      setAiBudget(getBudget());
  ...
  //
  // This pipeline lived inside `PayrollView`, which is why a payroll file dropped on Home
  // could only ever be ROUTED to that screen — the home promise is "drop anything here and
  // we handle the rest", and payroll was the one thing that answered "go to this other
  // page". Worse on the client seat: it was stashed unparsed for the accountant, so
  // dropping a register did nothing at all until someone else opened a different screen.
  //
  // ★ MOVED, NOT COPIED. A second implementation is the ·3a failure — two halves of one
  // contract drifting apart while both look tested. `PayrollView` now calls these from
  // context, so the drop zone, the Home queue and the manual Post button all run the same
  // code and cannot disagree about what a register does.
  // ═══════════════════════════════════════════════════════════════════════════
  const handlePayrollFile = async (file) => {
    ...
    try {
      ...
      logAudit("payroll_parsed", `${parsed.source} payroll parsed: ${fmtMoney(parsed.total_gross)} gross, ${(parsed.employees||[]).length} employees`);
      ...
      // ★★ THE REASONS GO INTO THE DURABLE ROW, NOT ONLY THE AUDIT LOG (C322). This used to
      // write "payroll imported — review/post in Payroll" and put WHY only in audit — so the
      // one record that survives a reload could not say what was wrong, and the Review card
      // that reads it (heldPayrollCards) would have had nothing to show but a filename.
      markIntake && markIntake(pIntakeId, INTAKE_STATUS.HELD, { detail: payrollHoldDetail(gate.reasons) });   // terminal: accounted for — and now on Review
      // ★ O116 — THE HELD CASE MUST STILL BE AN OUTCOME, NOT A SILENCE. The gate refusing is
      // the CORRECT result for a register that doesn't foot or is outside this company's
      // norms — but without a word here, a client who dropped payroll on Home would see the
      // file accepted and then nothing, forever, which is exactly what "handled" must not
      // mean. The reasons come from the gate, so this cannot describe a hold that isn't the
      // one that happened (§9).
      const why = (gate.reasons || []).map(r => r.text).filter(Boolean).join(" ");
      showNotification(navSeat.isReviewerSeat
  ...
  const postPayrollOnce = async (imp, { auto = false } = {}) => {
    ...
    try {
      if (imp._intakeId) {
        markIntake && markIntake(imp._intakeId, INTAKE_STATUS.RECORDED, { detail: `payroll posted — ${fmtMoney(imp.total_gross)} gross`, journalEntryIds: [String(jeId)] });
        ...
        if (rows && rows[0]) markIntake && markIntake(rows[0].id, INTAKE_STATUS.RECORDED, { detail: `payroll posted — ${fmtMoney(imp.total_gross)} gross`, journalEntryIds: [String(jeId)] });
      }
    } catch (e) { console.warn("[payroll] intake close-out skipped:", e?.message || e); }
    logAudit("payroll_posted", `${imp.source} payroll ${auto ? "auto-posted (register passed every shape check)" : "posted"}: ${fmtMoney(imp.total_gross)} gross → Dr Salaries/Tax · Cr Cash/Payroll Taxes Payable`, null, { auto, journal_entry_id: String(jeId) });
    ...
    showNotification((auto
      ...
      : `Payroll posted: ${fmtMoney(imp.total_gross)} gross ✓`)
  ...
  const routeFileToType = (type, file) => {
    // ★★ O116 — PAYROLL IS NO LONGER HANDED OVER, ON EITHER SEAT.
    //
    // The home screen says "drop anything here and we'll handle the rest", and payroll was
    // the one thing that answered "go to this other page" — or, on the CLIENT seat, "we've
    // saved that for your accountant", which meant a dropped register **did nothing at all**
    // until somebody else opened a different screen days later.
    //
    // It runs here now, on both seats, because the auto-post gate (·3a) already decides the
    // hard part: a standard register that proves itself POSTS, and anything that doesn't is
    // held for a person with the failing reasons attached. So the client gets an OUTCOME —
    // their payroll is in the books — or an honest hold, and neither requires them to
    // operate a screen they have no business operating.
    //
    // ▶ The other three keep routing DELIBERATELY. A bank statement opens a reconciliation
    // a person drives; a contract needs terms confirmed; QuickBooks is a mapping exercise.
    // Payroll is the one whose clean path is fully decided by a gate we already trust.
    if (type === "payroll") { handlePayrollFile(file); return; }

    if (!navSeat.isReviewerSeat && (type === "bank_statement" || type === "qbo")) {
  ...
  const handleContractFile = async (file) => {
    ...
    try {
      ...
      // O89 — the ASC 842 / ASC 606 entry generation moved to `contractEntries.js` so it can
      // be tested; the extraction call and the persistence stayed here. `calcASC842` was
      // already a tested library function — turning its schedule into JOURNAL ENTRIES was not,
      // and that is the half that decides what actually reaches the books.
      const planned = planContractEntries({ contract, rc, rn, today: todayLocal(), defaultIbr: DEFAULT_IBR });
      const monthlyEntries = planned.monthlyEntries;
...
${JSON.stringify(remainReceivables.map(i => ({ id: i.id, vendor: i.vendor, description: i.description, amount: i.amount, date: i.date, gl_code: i.gl_code, gl_name: i.gl_name, balance_remaining: i.balance_remaining || i.amount })))}` },
  ...
  const checkWatchTriggers = (newInvoices, currentUnknownDocs) => {
    ...
    setUnknownDocs(prev => prev.map(doc => {
      ...
      // C364 — the match is written to the row so a reload keeps it; a lost write is audited,
      // and the in-session copy still renders for this sitting.
      if (isDbUnknownDocId(doc.db_id) && currentCompany?.id) {
        checkedRowUpdate({ supabase, table: "unknown_documents", id: doc.db_id, companyId: currentCompany.id, patch: { watch_matches: newWatchMatches }, label: "unknownDocWatchMatch" })
          .then(r => { if (!r.ok) logAudit("unknown_doc_watch_persist_failed", `Couldn't record a watch match on ${doc.name} — it will be gone after a reload`, null, { doc_id: doc.db_id }); });
      }
  ...
  // `aliasIndex` is a dependency: confirming an alias (C281) must regroup the list at once,
  // not on the next booking.
  const vendorSummary = useMemo(() => buildVendorSummary(invoices, aliasIndex), [invoices, aliasIndex]);
  ...
  // ★ C347 — THIS WAS EXPOSED ON THE CONTEXT AND READ BY NOTHING. Its one reader was
  // InvoicesView, deleted in C335; C315 had repointed DetailView's "View all transactions
  // for X" at BooksView + this filter, and BooksView never read it — so the door opened
  // onto the UNFILTERED list for four days. BooksView reads it now.
  const filteredInvoices = useMemo(() => scopeInvoicesToVendor(invoices, vendorFilter, aliasIndex), [invoices, vendorFilter, aliasIndex]);
  ...
  return (
    ...
    <div style={{ fontFamily:"'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", minHeight:"100vh", background:"var(--sc-bg)", color:"var(--sc-text)" }}>
      ...
      {/* ★★ DOCUMENT READING IS PAUSED — SAY SO, ONCE, AT THE TOP.
          The O84 finding: the account ran out of credit mid-drive and every
          extraction-dependent feature failed with NO in-app explanation. Uploads still
          worked, files still arrived, and nothing worked afterwards — which reads as the
          product being broken rather than as a thing we know about.
          Only the two failures that genuinely stop the FEATURE raise this (`isDegradedMode`);
          one unreadable file never does. Dismissable, because the next successful call is
          not something this banner can observe. */}
      {aiDegraded && degradedBannerCopy(aiDegraded.kind) && (
        <div role="status" style={{ flexShrink:0, background:"var(--sc-warning-soft)", borderBottom:"1px solid var(--sc-warning-soft)", color:"var(--sc-warning)", padding:"11px 24px", display:"flex", alignItems:"center", gap:12, fontSize:13, fontWeight:600, zIndex:51 }}>
          <span style={{ fontSize:16 }}>⏸</span>
          <span style={{ flex:1 }}>{degradedBannerCopy(aiDegraded.kind)}</span>
          <button onClick={()=>setAiDegraded(null)} style={{ background:"none", border:"1px solid currentColor", borderRadius:6, padding:"3px 10px", color:"inherit", fontSize:11, cursor:"pointer" }}>Dismiss</button>
        </div>
      )}
