...
describe("deadlineIsWaiting", () => {
  ...
  const filing = getTaxDeadlines(new Date(2026, 0, 20)).find((d) => !d.est);
  ...
  it("Home and the bell both read it", () => {
    ...
    expect(home).toMatch(/const dl = deadlineIsWaiting\(dl0, est\) \? dl0 : null;/);
    expect(app).toMatch(/const nextDue = deadlineIsWaiting\(nextDue0, est\) \? nextDue0 : null;/);
  });
});
