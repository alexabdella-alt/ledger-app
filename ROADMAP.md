...
| O57 | Credit-card offset binding — bank imports aren't bound to a specific account, so the direct-book offset is hardcoded `Cr Cash (1000)`. A credit-card statement's charges should post `Dr Expense / Cr Credit Card Liability (2200)`. Bind the importer to the selected `bank_account`'s `gl_code`/`type` so the offset follows the account | open bug | **P2** | any client with a business credit card hits this; offset is currently `rc("cash")` regardless (see C60/`buildBankLineEntry`) |
