...
- **Test suite:** 418 passing (`npm test`, 30 files). **Build:** clean (`npm run build`).
- **Migrations:** `000`–`044` (numbering non-contiguous; no new migrations in the C64–C69 pass — all code-only).
...
| O70 | **Reports date range should auto-default to the current date on open** — the "to" date must always be **today** (not a stale saved value), with "from" defaulting to the relevant period start per the selected view (month-to-date / year-to-date / fiscal-year-start). Today it appears to persist a stale date rather than tracking the current date, so reports can silently open on the wrong window. | not started | P2 | ties **O20** (report date semantics: range/asOf consistency) + the fiscal-year-boundary work; "from" should derive from the shared `fiscalYearStart`/period helper so it matches "current period" everywhere |
