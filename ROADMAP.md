...
- **Migrations:** `000`–`044` applied; **`045_drop_ap_invoices.sql` WRITTEN this pass but NOT yet applied** — it's self-guarded (refuses to drop if the table has rows) and awaits the user's row-count confirmation + manual apply (O19).
...
| O19 | Drop orphaned `ap_invoices` table (zero references) after confirming empty/unused | **migration written — awaiting apply** | P3 | **Audit (2026-06-26):** ZERO `src/` references confirmed (grep); only the table's own DDL + the dead `ap_aging` view read it (no app code uses that view either — aging is client-side in `reports.js`); `ar_aging`/`trial_balance` are independent. **`045_drop_ap_invoices.sql` written** — self-guarded (RAISES + aborts if the table has any rows; drops the `ap_aging` view first). **ACTION (you):** run `select count(*) from ap_invoices;` to confirm 0 rows, then apply 045 in the Supabase SQL editor. Migration is a no-op/abort if non-empty. CLAUDE.md §11 |
