...
| O69 | **`bookBankTransactions` reports success without persisting (false-completeness + card-vs-AP gap).** Surfaced by the O41/cards shakedown. **(B) FALSE SUCCESS:** `applyMatch` records "AP Cleared / payment posted ✓" into `matchHistory` ("Cleared This Session") while **ignoring `markBillPaid`'s return value** — `markBillPaid` returns `false` (no JE) for a local-only id, yet the UI claims success. **(A) PERSISTENCE:** `bookBankTransactions` writes charges to local React state (`setInvoices`) only — **no `bookToDb`/`post_journal_entry`** — so direct-booked charges never become DB journal entries and **vanish on refresh**. Net live result: UI claims success, **zero journal entries written**. This is a **silent-disappearance / false-completeness instance — ties O60** (a "booked in the UI but no JE / no trace" outcome the completeness guarantee must make impossible). **(D) OFFSET:** the path hardcodes `secondary_gl_code: rc("cash")` and never threads the selected account's GL — the O57/C63 offset binding (Cr 2200 for a card) was wired into `buildBankLineEntry`/the universal `planBankImport` path/`dismissMatch` but **never into `bookBankTransactions`** (`handleBankFile` even computes `offsetCode`/`offsetName` then discards them). **(C) CONCEPTUAL:** credit-card imports run the **same bank-style AP-matching** as a bank account (no `account.type` check) — a **card charge ≠ an AP payment** (a charge *creates* a liability, Dr Expense / Cr 2200; it doesn't *clear* a payable). **Fix order B→A→D→C:** B = `applyMatch` must verify every `markBillPaid` committed before recording cleared/showing "posted ✓" (else leave in review, surface error); A = persist direct-booked charges via `bookToDb`/`persistMultiLineEntry`; D = offset by the import account (card→2200, bank→1000) via `buildBankLineEntry`; C = branch on `account.type` — credit_card → skip matching + direct-book Dr Expense / Cr 2200, bank → keep AP-matching; keep the rare "paid a vendor bill by card" as explicit opt-in (note: **Dr A/P / Cr 2200**, NOT Cr Cash → `markBillPaid`/`buildPaymentEntry` must accept a non-cash offset). **BUILT (B→A→D→C) — pending live verify:** B = `applyMatch` now checks every `markBillPaid` via `allClearingsPosted([...])` and bails (left in review, error toast) if any didn't commit — never claims an uncommitted write; A = `bookBankTransactions` direct-books every line through `bookToDb` (post_journal_entry); D = offset threaded from the import account (card→2200/bank→1000) via `buildBankLineEntry`; C = `shouldRunApMatching(account)` — credit_card → skip matching + direct-book Dr Expense / Cr 2200, bank → `planBankImport` split (clears post-only, standalone persisted, no double-count). Tests: `allClearingsPosted`/`shouldRunApMatching` + card-vs-bank offset locks in `tests/bankMatch.test.js`. **Deferred (explicit opt-in):** the rare "paid a vendor bill by card" (Dr A/P / Cr 2200) — needs `buildPaymentEntry`/`markBillPaid` to take a non-cash offset + a per-line UI action. | built — pending live verify | **P1** | **trust-critical**; spans **O57** (offset), **O60** (false-completeness/silent-disappearance), and the matching logic; `bookBankTransactions`/`applyMatch` in App.jsx, `buildBankLineEntry`/`planBankImport`/`allClearingsPosted`/`shouldRunApMatching` in bankMatch.js |
