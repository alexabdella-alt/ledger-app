...
**Fiscal-year / Retained-Earnings model (decided):** the balance sheet currently lumps **all-time** net income into one "current period" line and never rolls completed years into RE — it stays balanced but misattributes prior-year earnings from a client's 2nd fiscal year on (Balance-Sheet net income then diverges from the Income Statement). Fix = **derived "soft close" (Option A):** at report time, key off `companies.fiscal_year_end` — Retained Earnings (beginning of current FY) = posted RE (3100) + net income of all completed fiscal years before the current FY; Net Income (current period) = revenue−expense within the current FY only; the two shown as separate lines (GAAP-standard). No posted closing entries required for the fix.

**Deferred / specified-but-not-built:** **hard close** (actually posting #17 closing entries + period locking) — build when period-locking becomes a real need; pairs with reconciliation locking. The soft-close derivation is the correctness fix; hard-close is the future locking feature.
